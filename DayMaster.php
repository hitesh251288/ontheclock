...
$query = "SELECT id FROM tgroup WHERE DNT = 'Yes' AND NightFlag = 1 AND id > 2";
$result = selectData($conn, $query);
$egrpValue = $result[0];
$query = "UPDATE tenter, tgate, tgroup SET tenter.e_group = " . (int)$result[0] . " WHERE tenter.g_id = tgate.id AND tgate.name LIKE '%(N)%' AND tenter.p_flag = 0 AND tgroup.id = tenter.e_group AND tgroup.DNT = 'Yes' AND tenter.e_date > '" . $txtLockDate . "' ";
updateIData($iconn, $query, true);
$query = "UPDATE tenter, tgate SET tenter.p_flag = 1 WHERE tenter.g_id = tgate.id AND tgate.Meal = 1 AND tenter.p_flag = 0 AND tenter.e_date > '" . $txtLockDate . "' ";
updateIData($iconn, $query, true);
...
while ($process_date <= insertToday()) {
    ...
    if ($missed_shift_rotation_flag) {
        $missed_shift_rotation_query = "SELECT DISTINCT(idf) FROM ShiftChangeMaster WHERE RotateShiftNextDay = " . $maxDate;
        ...
        while ($missed_shift_rotation_cur = mysqli_fetch_row($missed_shift_rotation_result)) {
            if (is_numeric($missed_shift_rotation_cur[0]) == true && 0 < $missed_shift_rotation_cur[0]) {
                exec("php RotateShift.php " . $missed_shift_rotation_cur[0]);
                $query = "INSERT INTO ProcessLog (PType, PDate, PTime) VALUES ('Auto Shift Rotation', " . $maxDate . ", '" . getNow() . "')";
                updateIData($iconn, $query, true);
            }
        }
    }
    if ($rotateShift == "Yes") {
        $query = "SELECT DISTINCT(idf), RotateShiftNextDay FROM ShiftChangeMaster WHERE AE = 1 AND SRScenario = 'Morning - 2 Shifts (No Day Shift on Rotation Day)' ";
        $result = mysqli_query($conn, $query);
        while ($cur = mysqli_fetch_row($result)) {
            $query = "UPDATE tenter SET e_group = (SELECT DISTINCT(tgroup.id) FROM tgroup, ShiftChangeMaster WHERE tgroup.id = ShiftChangeMaster.id AND ShiftChangeMaster.idf = " . $cur[0] . " AND tgroup.NightFlag = 1) WHERE p_flag = 0 AND e_etc <> 'P' AND e_group = (SELECT DISTINCT(tgroup.id) FROM tgroup, ShiftChangeMaster WHERE tgroup.id = ShiftChangeMaster.id AND ShiftChangeMaster.idf = " . $cur[0] . " AND tgroup.NightFlag = 0) AND e_date = '" . getLastDay($cur[1], 7) . "' ";
            updateIData($iconn, $query, true);
            $query = "UPDATE tenter, tuser, ShiftChangeMaster SET tenter.e_group = tuser.group_id WHERE tenter.e_id = tuser.id AND tenter.p_flag = 0 AND (tenter.e_etc <> 'P' OR tenter.e_etc IS NULL) AND tenter.e_date > '" . getLastDay($cur[1], 7) . "' AND tenter.e_date < '" . $cur[1] . "' AND tuser.group_id = ShiftChangeMaster.id AND ShiftChangeMaster.idf = '" . $cur[0] . "' ";
            updateIData($iconn, $query, true);
        }
        $query = "SELECT DISTINCT(idf), RotateShiftNextDay, RTime FROM ShiftChangeMaster WHERE AE = 1 AND SRScenario = 'Morning - 2 Shifts' ";
        $result = mysqli_query($conn, $query);
        while ($cur = mysqli_fetch_row($result)) {
            $eid = "";
            $query = "SELECT tenter.ed, tenter.e_id, tenter.e_time FROM tenter, tuser WHERE tenter.e_id = tuser.id AND tenter.p_flag = 0 AND (tenter.e_etc <> 'P' OR tenter.e_etc IS NULL) AND tenter.e_date = '" . getLastDay($cur[1], 7) . "' AND tuser.group_id = (SELECT DISTINCT(tgroup.id) FROM tgroup, ShiftChangeMaster WHERE tgroup.id = ShiftChangeMaster.id AND ShiftChangeMaster.idf = " . $cur[0] . " AND tgroup.NightFlag = 0) ORDER BY tenter.e_id, tenter.e_time ";
            ...
            while ($sub_cur = mysqli_fetch_row($sub_result)) {
                if ($eid != $sub_cur[1]) {
                    ...
                    $query = "UPDATE tenter SET e_group = (SELECT DISTINCT(tgroup.id) FROM tgroup, ShiftChangeMaster WHERE tgroup.id = ShiftChangeMaster.id AND ShiftChangeMaster.idf = " . $cur[0] . " AND tgroup.NightFlag = 0) WHERE ((e_time > '" . $cur[2] . "' AND e_date = " . getLastDay($cur[1], 7) . ") OR (e_date > " . getLastDay($cur[1], 7) . " AND e_date <= " . $cur[1] . ")) AND e_id = '" . $sub_cur[1] . "' AND p_flag = 0 AND e_etc <> 'P' ";
                    updateIData($iconn, $query, true);
                    $query = "UPDATE tenter SET e_group = (SELECT DISTINCT(tgroup.id) FROM tgroup, ShiftChangeMaster WHERE tgroup.id = ShiftChangeMaster.id AND ShiftChangeMaster.idf = " . $cur[0] . " AND tgroup.NightFlag = 1) WHERE e_id = '" . $sub_cur[1] . "' AND p_flag = 0 AND e_etc <> 'P' AND e_date > " . getLastDay($cur[1], 14) . " AND e_date < " . getLastDay($cur[1], 7);
                    updateIData($iconn, $query, true);
                }
                $eid = $sub_cur[1];
            }
            ...
            updateIData($iconn, $query, true);
            $query = "UPDATE tenter AS A, tuser AS B SET A.e_group = (SELECT DISTINCT(tgroup.id) FROM tgroup, ShiftChangeMaster WHERE tgroup.id = ShiftChangeMaster.id AND ShiftChangeMaster.idf = " . $cur[0] . " AND tgroup.NightFlag = 0) WHERE A.e_id = B.id AND A.p_flag = 0 AND A.e_etc <> 'P' AND B.group_id = (SELECT DISTINCT(tgroup.id) FROM tgroup, ShiftChangeMaster WHERE tgroup.id = ShiftChangeMaster.id AND ShiftChangeMaster.idf = " . $cur[0] . " AND tgroup.NightFlag = 1) AND A.e_date < '" . getLastDay($cur[1], 7) . "' AND A.e_date > '" . getLastDay($cur[1], 14) . "' ";
            updateIData($iconn, $query, true);
    ...
    if ($lstUseShiftRoster == "Yes" || $lstCAGR == "Yes") { 
        ...
        while ($cur = mysqli_fetch_row($result)) {
            ...
            while ($sub_cur = mysqli_fetch_row($sub_result)) {
                $dayNight = true;
                if ($sub_cur[2] < $txtNightShiftMaxOutTime . "00" && ($counter == 0 || $etime != "" && mktime(substr($sub_cur[2], 0, 2), substr($sub_cur[2], 2, 2), substr($sub_cur[2], 4, 2), substr($nextDate, 4, 2), substr($nextDate, 6, 2), substr($nextDate, 0, 4)) - mktime(substr($etime, 0, 2), substr($etime, 2, 2), substr($etime, 4, 2), substr($nextDate, 4, 2), substr($nextDate, 6, 2), substr($nextDate, 0, 4)) < $txtMinClockinPeriod)) {
...
//        echo "<pre>";print_R($cur);die;
    ...
    if ($insertOk) {
            //if (insertAttendance($conn, $iconn, $cur[0], displayDate($cur[3]), displayDate($cur[3]), $insert_shift, $cur[2], $cur[1])) {
                if ($cur[5] != "." && $cur[5] != "") {
                    $query = "UPDATE AttendanceMaster SET Remark = '" . $cur[5] . "' WHERE EmployeeID = '" . $cur[0] . "' AND ADate = '" . $cur[3] . "' AND Flag = '" . $cur[2] . "'";
                    updateIData($iconn, $query, true);
                }
                $query = "UPDATE AttendanceMaster SET AOvertime = '" . $cur[8] * 3600 . "' WHERE EmployeeID = '" . $cur[0] . "' AND ADate = '" . $cur[3] . "' AND Flag = '" . $cur[2] . "'";
                updateIData($iconn, $query, true);
                if ($cur[4] == 1 && $maxDate == $cur[3]) {
                    rotateShift($conn, $iconn, $cur[0], $cur[7]);
                }
                $recStat = true;
            }
        }
        if ($recStat) {
            $query = "UPDATE FlagDayRotation SET RecStat = 1 WHERE e_id = " . $cur[0] . " AND e_date = " . $cur[3];
            updateIData($iconn, $query, true);
        }
    }
    $query = "SELECT DrillMasterID, DrillDate, DrillTimeFrom, DrillTimeTo FROM DrillMaster WHERE DrillDate < '" . $maxDate . "' AND DrillDate >= '" . $last_process_date . "'";
    $drill_result = mysqli_query($conn, $query);
    while ($drill_cur = mysqli_fetch_row($drill_result)) {
        $lstDrill = $drill_cur[0];
        $main_query = "UPDATE tenter, tuser SET p_flag = 1 WHERE tenter.e_id = tuser.id AND tenter.e_date = '" . $drill_cur[1] . "' AND tenter.e_time >= '" . $drill_cur[2] . "' AND tenter.e_time <= '" . $drill_cur[3] . "' ";
    ...
    while ($sdcur = mysqli_fetch_row($result)) { 
        ...
        while ($sdcur1 = mysqli_fetch_row($result1)) {
            ...
            while ($cur = mysqli_fetch_row($result2)) {
                if ($time == "") {
                    $date = $cur[0];
                    $time = mktime(substr($cur[1], 0, 2), substr($cur[1], 2, 2), substr($cur[1] . "00", 4, 2), substr($cur[0], 4, 2), substr($cur[0], 6, 2), substr($cur[0], 0, 4));
                    $gate = $cur[2];
                    $a0[$dayCount] = $cur[0];
                    $a1[$dayCount] = $cur[1];
                    $a2[$dayCount] = $cur[2];
                    ...
                    $this_time = mktime(substr($cur[1], 0, 2), substr($cur[1], 2, 2), substr($cur[1] . "00", 4, 2), substr($cur[0], 4, 2), substr($cur[0], 6, 2), substr($cur[0], 0, 4));
                    if ($cur[0] == $date) {
                        ...
                        if ($dayCount % 2 == 0) {
                            ...
                            if ($exitClocking == 0 || $exitCount == 0) {
                                ...
                                while ($this_i < count($a1) - 1) {
                                    $this_work = $this_work + mktime(substr($a1[$this_i + 1], 0, 2), substr($a1[$this_i + 1], 2, 2), substr($a1[$this_i + 1], 4, 2), substr($a0[$this_i + 1], 4, 2), substr($a0[$this_i + 1], 6, 2), substr($a0[$this_i + 1], 0, 4)) - mktime(substr($a1[$this_i], 0, 2), substr($a1[$this_i], 2, 2), substr($a1[$this_i], 4, 2), substr($a0[$this_i], 4, 2), substr($a0[$this_i], 6, 2), substr($a0[$this_i], 0, 4));
                                    $this_i = $this_i + 2;
                                }
                                if ($a1[0] != "" && $a1[$dayCount - 1] != "") {
                                    $query = "INSERT IGNORE INTO DayMaster (e_id, TDate, Entry, Start, BreakOut, BreakIn, Close, DayMaster.Exit, group_id, Flag, Work) VALUES (" . $sdcur1[0] . ", " . $date . ", '" . $a1[0] . "', '" . $a1[0] . "', '" . $a1[0] . "', '" . $a1[0] . "', '" . $a1[$dayCount - 1] . "', '" . $a1[$dayCount - 1] . "', " . $sdcur[0] . ", '" . $pflag . "', " . $this_work . ")";
                                    $process_flag = true;
                                }
                            } else {
                                $this_work = 0;
                                ...
                                if ($a1[1] != "" && $a1[$dayCount - 2] != "") {
                                    $query = "INSERT IGNORE INTO DayMaster (e_id, TDate, Entry, Start, BreakOut, BreakIn, Close, DayMaster.Exit, group_id, Flag, Work) VALUES (" . $sdcur1[0] . ", " . $date . ", '" . $a1[0] . "', '" . $a1[1] . "', '" . $a1[1] . "', '" . $a1[1] . "', '" . $a1[$dayCount - 2] . "', '" . $a1[$dayCount - 1] . "', " . $sdcur[0] . ", '" . $pflag . "', " . $this_work . ")";
                                    $process_flag = true;
                                }
                            }
                            if (updateIData($iconn, $query, true)) {
...
//                echo "\n";
            ...
            while ($cur = mysqli_fetch_row($result2)) {
                if ($time == "") {
                    $date = $cur[0];
                    $time = mktime((int) substr($cur[1], 0, 2), (int) substr($cur[1], 2, 2), (int) substr($cur[1] . "00", 4, 2), (int) substr($cur[0], 4, 2), (int) substr($cur[0], 6, 2), (int) substr($cur[0], 0, 4));
                    $gate = $cur[2];
                    $a0[$dayCount] = $cur[0];
                    $a1[$dayCount] = $cur[1];
                    $a2[$dayCount] = $cur[2];
                        ...
                        if (($exitCount == $exitClocking || $exitClocking == 0 || $exitCount == 0 && $lstNoExitException == "Yes") && ($deptCount == $deptClocking || $deptCount == 2 && $lstNoBreakException == "Yes" || $deptCount == 2 && $noBreakFlag == true || ($s_result[0] == 5 || $s_result[0] == 6) && 1 < $deptCount)) {
                            ...
                            if ($exitClocking == 0 || $exitCount == 0) {
                                if ($deptCount == 2 || $s_result[0] == 5 || $s_result[0] == 6) {
                                    if ($a1[0] != "" && $a1[$dayCount - 1] != "") {
                                        $query = "INSERT IGNORE INTO DayMaster (e_id, TDate, Entry, Start, BreakOut, BreakIn, Close, DayMaster.Exit, group_id, Flag) VALUES (" . $sdcur1[0] . ", " . $date . ", '" . $a1[0] . "', '" . $a1[0] . "', '" . $a1[0] . "', '" . $a1[0] . "', '" . $a1[$dayCount - 1] . "', '" . $a1[$dayCount - 1] . "', " . $sdcur[0] . ", '" . $pflag . "')";
                                        $process_flag = true;
                                    }
                                } else {
                                    if ($a1[0] != "" && $a1[3] != "") {
                                        $query = "INSERT IGNORE INTO DayMaster (e_id, TDate, Entry, Start, BreakOut, BreakIn, Close, DayMaster.Exit, group_id, Flag) VALUES (" . $sdcur1[0] . ", " . $date . ", '" . $a1[0] . "', '" . $a1[0] . "', '" . $a1[1] . "', '" . $a1[2] . "', '" . $a1[3] . "', '" . $a1[3] . "', " . $sdcur[0] . ", '" . $pflag . "')";
                                        $process_flag = true;
                                    }
                                }
                            } else {
                                if ($deptCount == 2 || $s_result[0] == 5 || $s_result[0] == 6) {
                                    if ($a1[1] != "" && $a1[$dayCount - 1] != "") {
                                        $query = "INSERT IGNORE INTO DayMaster (e_id, TDate, Entry, Start, BreakOut, BreakIn, Close, DayMaster.Exit, group_id, Flag) VALUES (" . $sdcur1[0] . ", " . $date . ", '" . $a1[0] . "', '" . $a1[1] . "', '" . $a1[1] . "', '" . $a1[1] . "', '" . $a1[$dayCount - 2] . "', '" . $a1[$dayCount - 1] . "', " . $sdcur[0] . ", '" . $pflag . "')";
                                        $process_flag = true;
                                    }
                                } else {
                                    if ($a1[1] != "" && $a1[4] != "") {
                                        $query = "INSERT IGNORE INTO DayMaster (e_id, TDate, Entry, Start, BreakOut, BreakIn, Close, DayMaster.Exit, group_id, Flag) VALUES (" . $sdcur1[0] . ", " . $date . ", '" . $a1[0] . "', '" . $a1[1] . "', '" . $a1[2] . "', '" . $a1[3] . "', '" . $a1[4] . "', '" . $a1[5] . "', " . $sdcur[0] . ", '" . $pflag . "')";
                                        $process_flag = true;
                                    }
                                }
                            }
            ...
            while ($cur = mysqli_fetch_row($result2)) {
                if ($time == "" && $cur[6] == 1) {
                    ...
                    if ($cur[0] . $cur[1] < $cut_off) {
                        ...
                        if ($cur[6] == 1) {
                            $last_time = mktime(substr($a1[$dayCount - 1], 0, 2), substr($a1[$dayCount - 1], 2, 2), substr($a1[$dayCount - 1], 4, 2), substr($a0[$dayCount - 1], 4, 2), substr($a0[$dayCount - 1], 6, 2), substr($a0[$dayCount - 1], 0, 4));
...
//            $last_time = mktime(substr($a1[$dayCount - 1], 0, 2), substr($a1[$dayCount - 1], 2, 2), substr($a1[$dayCount - 1], 4, 2), substr($a0[$dayCount - 1], 4, 2), substr($a0[$dayCount - 1], 6, 2), substr($a0[$dayCount - 1], 0, 4));
            $last_time = mktime(
                substr($a1[$dayCount - 1], 0, 2),
                substr($a1[$dayCount - 1], 2, 2),
                substr($a1[$dayCount - 1], 4, 2),
                substr($a0[$dayCount - 1], 4, 2),
                substr($a0[$dayCount - 1], 6, 2),
                substr($a0[$dayCount - 1], 0, 4)
            );
            ...
            while ($sdcur1 = mysqli_fetch_row($result1)) {
                ...
                while ($cur = mysqli_fetch_row($result2)) {
                    if ($firstcount == 0 - 1 && $txtNightShiftMaxOutTime . "00" < $cur[1] && $date < $cur[0]) {
                        $firstcount = 0;
                        $next_time = mktime(substr($txtNightShiftMaxOutTime, 0, 2), substr($txtNightShiftMaxOutTime, 2, 2), "00", substr($cur[0], 4, 2), substr($cur[0], 6, 2), substr($cur[0], 0, 4)) + 86400;
                        $time = mktime(substr($cur[1], 0, 2), substr($cur[1], 2, 2), substr($cur[1] . "00", 4, 2), substr($cur[0], 4, 2), substr($cur[0], 6, 2), substr($cur[0], 0, 4));
                        $gate = $cur[2];
                        $a0[$dayCount] = $cur[0];
                        $a1[$dayCount] = $cur[1];
                        ...
                        if ($time != "" && $firstcount == 0) {
                            if ($this_time < $next_time) {
                                ...
                                if ($txtNightShiftMaxOutTime . "00" < $cur[1] && $date < $cur[0]) {
                                    $next_time = mktime(substr($txtNightShiftMaxOutTime, 0, 2), substr($txtNightShiftMaxOutTime, 2, 2), "00", substr($date, 4, 2), substr($date, 6, 2), substr($date, 0, 4)) + 86400;
                                    $time = $this_time;
                                    $gate = $cur[2];
                                    $a0[$dayCount] = $date;
                                    $a1[$dayCount] = $cur[1];
    ...
    while ($cur = mysqli_fetch_row($result)) {
        ...
        while ($sdcur1 = mysqli_fetch_row($result1)) {
            ...
            $query = "SELECT DayMaster.DayMasterID, DayMaster.TDate, DayMaster.Entry, DayMaster.Start, DayMaster.BreakOut, DayMaster.BreakIn, DayMaster.Close, DayMaster.Exit, DayMaster.group_id, DayMaster.Flag, DayMaster.Work FROM DayMaster WHERE DayMaster.p_flag = 0 AND DayMaster.e_id = " . $sdcur1[0] . " AND DayMaster.group_id = " . $cur[0] . " ORDER BY DayMaster.DayMasterID";
            $result2 = mysqli_query($kconn, $query);
            while ($cur2 = mysqli_fetch_row($result2)) {
                ...
                $start = mktime((int)substr($cur2[3], 0, 2), (int)substr($cur2[3], 2, 2), (int)substr($cur2[3], 4, 2), (int)substr($cur2[1], 4, 2), (int)substr($cur2[1], 6, 2), (int)substr($cur2[1], 0, 4));
                ...
                $ibin = 0;
                $iclose = 0;
                $istart = mktime(substr($cur[2], 0, 2), substr($cur[2], 2, 2), 0, substr($cur2[1], 4, 2), substr($cur2[1], 6, 2), substr($cur2[1], 0, 4));
                $igrace = mktime(substr($cur[3], 0, 2), substr($cur[3], 2, 2), 0, substr($cur2[1], 4, 2), substr($cur2[1], 6, 2), substr($cur2[1], 0, 4));
                if ($cur2[4] < $cur2[3]) {
                    ...
                    if (strlen($cur[5]) == 4) {
                        $ibout = mktime(substr($cur[5], 0, 2), substr($cur[5], 2, 2), 0, $m, $d, $a["year"]);
                    }
                } else {
                    ...
                    if (strlen($cur[5]) == 4) {
                        $ibout = mktime(substr($cur[5], 0, 2), substr($cur[5], 2, 2), 0, substr($cur2[1], 4, 2), substr($cur2[1], 6, 2), substr($cur2[1], 0, 4));
                    }
                }
                if ($cur2[5] < $cur2[3]) {
                    $next = strtotime(substr($cur2[1], 6, 2) . "-" . substr($cur2[1], 4, 2) . "-" . substr($cur2[1], 0, 4) . " + 1 day");
                    ...
                    $bin = mktime(substr($cur2[5], 0, 2), substr($cur2[5], 2, 2), substr($cur2[5], 4, 2), $m, $d, $a["year"]);
                    if (strlen($cur[6]) == 4) {
                        $ibin = mktime(substr($cur[6], 0, 2), substr($cur[6], 2, 2), 0, $m, $d, $a["year"]);
                    }
                } else {
                    $bin = mktime((int)substr($cur2[5], 0, 2), (int)substr($cur2[5], 2, 2), (int)substr($cur2[5], 4, 2), (int)substr($cur2[1], 4, 2), (int)substr($cur2[1], 6, 2), (int)substr($cur2[1], 0, 4));
                    if (strlen($cur[6]) == 4) {
                        $ibin = mktime(substr($cur[6], 0, 2), substr($cur[6], 2, 2), 0, substr($cur2[1], 4, 2), substr($cur2[1], 6, 2), substr($cur2[1], 0, 4));
                    }
                }
                if ($cur2[6] < $cur2[3]) {
                    $next = strtotime(substr($cur2[1], 6, 2) . "-" . substr($cur2[1], 4, 2) . "-" . substr($cur2[1], 0, 4) . " + 1 day");
                    ...
                    $close = mktime((int)substr($cur2[6], 0, 2), (int)substr($cur2[6], 2, 2), (int)substr($cur2[6], 4, 2), (int)$m, (int)$d, (int)$a["year"]);
                    if (strlen($cur[7]) == 4) {
                        $iclose = mktime(substr($cur[7], 0, 2), substr($cur[7], 2, 2), 0, $m, $d, $a["year"]);
                    }
                } else {
                    if ($cur[8] == 0) {
                        $close = mktime(substr($cur2[6], 0, 2), substr($cur2[6], 2, 2), substr($cur2[6], 4, 2), substr($cur2[1], 4, 2), substr($cur2[1], 6, 2), substr($cur2[1], 0, 4));
                        if (strlen($cur[7]) == 4) {
                            $iclose = mktime(substr($cur[7], 0, 2), substr($cur[7], 2, 2), 0, substr($cur2[1], 4, 2), substr($cur2[1], 6, 2), substr($cur2[1], 0, 4));
                        }
                    } else {
                        $close = mktime(substr($cur2[6], 0, 2), substr($cur2[6], 2, 2), substr($cur2[6], 4, 2), substr($cur2[1], 4, 2), substr($cur2[1], 6, 2), substr($cur2[1], 0, 4));
                        $next = strtotime(substr($cur2[1], 6, 2) . "-" . substr($cur2[1], 4, 2) . "-" . substr($cur2[1], 0, 4) . " + 1 day");
    ...
    if ($o_result[0] == 1) {
        ...
        while ($cur = mysqli_fetch_row($result)) {
            ...
            while ($this_cur = mysqli_fetch_row($this_result)) {
                ...
                $lstDeptTerminal = 0;
                if (is_array($sub_result) && isset($sub_result[0])) {
                    $lstDeptTerminal = $sub_result[0];
                }
    ...
    while ($cur = mysqli_fetch_row($result)) {
        ...
        if ($last_id != "") {
            ...
            if ($nextDate < $maxDate) {
                $carryover = mktime(substr($cur[6], 0, 2), substr($cur[6], 2, 2), substr($cur[6], 4, 2), substr($nextDate, 4, 2), substr($nextDate, 6, 2), substr($nextDate, 0, 4)) - mktime(substr($last_start, 0, 2), substr($last_start, 2, 2), 0, substr($nextDate, 4, 2), substr($nextDate, 6, 2), substr($nextDate, 0, 4));
                ...
                if (updateIData($iconn, $query, true)) {
                    ...
                    if (updateIData($iconn, $query, true)) {
                        ...
                        if (updateIData($iconn, $query, true)) {
                            ...
                            if (updateIData($iconn, $query, true)) {
                                $query = "INSERT INTO DayMaster (e_id, TDate, Entry, Start, BreakOut, BreakIn, Close, DayMaster.Exit, group_id, Flag, p_flag) VALUES (" . $cur[2] . ", " . $nextDate . ", '" . $last_start . "01', '" . $last_start . "01', '" . $last_start . "01', '" . $last_start . "01', '" . $cur[6] . "', '" . $cur[6] . "', " . $last_id . ", 'Proxy', 1)";
                                if (updateIData($iconn, $query, true)) {
...
if (getRegister($txtMACAddress, 7) == "25" || getRegister($txtMACAddress, 7) == "86" || getRegister($txtMACAddress, 7) == "44" || getRegister($txtMACAddress, 7) == "43") {
    ...
    $result = mysqli_query($conn, $query);
    while ($cur = mysqli_fetch_row($result)) {
        $a = mktime(substr($cur[0], 0, 2), substr($cur[0], 2, 2), substr($cur[0], 4, 2), substr($cur[2], 4, 2), substr($cur[2], 6, 2), substr($cur[2], 0, 4));
        $b = mktime(substr($cur[5], 0, 2), substr($cur[5], 2, 2), "00", substr($cur[2], 4, 2), substr($cur[2], 6, 2), substr($cur[2], 0, 4));
        $c = $a - $b;
        $d = mktime(substr($cur[1], 0, 2), substr($cur[1], 2, 2), substr($cur[1], 4, 2), substr($cur[2], 4, 2), substr($cur[2], 6, 2), substr($cur[2], 0, 4));
        $e = mktime(substr($cur[6], 0, 2), substr($cur[6], 2, 2), "00", substr($cur[2], 4, 2), substr($cur[2], 6, 2), substr($cur[2], 0, 4));
        $f = $d - $e;
    ...
    while ($cur = mysqli_fetch_row($result)) {
        $a = mktime(substr($cur[0], 0, 2), substr($cur[0], 2, 2), substr($cur[0], 4, 2), substr($cur[2], 4, 2), substr($cur[2], 6, 2), substr($cur[2], 0, 4));
        $b = mktime(substr($cur[7], 0, 2), substr($cur[7], 2, 2), "00", substr($cur[2], 4, 2), substr($cur[2], 6, 2), substr($cur[2], 0, 4));
        $c = $a - $b;
        $d = mktime(substr($cur[1], 0, 2), substr($cur[1], 2, 2), substr($cur[1], 4, 2), substr($cur[2], 4, 2), substr($cur[2], 6, 2), substr($cur[2], 0, 4));
        $e = mktime(substr($cur[8], 0, 2), substr($cur[8], 2, 2), "00", substr($cur[2], 4, 2), substr($cur[2], 6, 2), substr($cur[2], 0, 4));
        $f = $d - $e;
        $sub_query = "SELECT AttendanceMaster.Normal, AttendanceMaster.Overtime, AttendanceMaster.AOvertime, AttendanceMaster.Day, AttendanceMaster.OT1, AttendanceMaster.OT2, AttendanceMaster.Flag FROM AttendanceMaster WHERE EmployeeID = '" . $cur[3] . "' AND AttendanceMaster.ADate = '" . $cur[2] . "' AND EarlyIn_flag <> 2 ";
        $sub_result = selectData($conn, $sub_query);
        $aot = $sub_result[2];
...
//                                    echo "Hey".$otCutoff =  strtotime($otcuttRow['EmpClose'].'00') - strtotime(displayVirdiTime($cur[3]))."<br>";
                    $otCutoff = strtotime(displayVirdiTime($cur['Exit'])) - strtotime(displayVirdiTime($otcuttRow['EmpClose'] . '00')) . "<br>";
