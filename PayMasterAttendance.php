...
for ($iii = 0; $iii < $main_count; $iii++) {
   ...
   //$oconn = mssql_connection($txtDBIP, $txtDBName, $txtDBUser, $txtDBPass);admin@123dsdfsdf
//    $oconn = mssql_connection("SERVER2", "PAY", "sa", "admin@123");
    //$oconn = mssql_connection("DESKTOP-I1EBN6N\SQLEXPRESS", "EnquestWeb", "sa", "namaste");
    $serverName = "DESKTOP-M5OA37B\SQLEXPRESS";
        $connectionOptions = array(
        "Database" => "iASWebSetup",      // your DB name
        "Uid"      => "sa",          // your SQL Server username
        "PWD"      => "sss", // your SQL Server password
        // "CharacterSet" => "UTF-8"    // ensures proper encoding
    );

    // Attempt to connect
    $oconn = sqlsrv_connect($serverName, $connectionOptions);
    if ($oconn) {
    ...
    $query = "ALTER TABLE dbo.tblEmpWorkHours_VIRDI ADD COLUMN LVOFF18_COUNT8 INT NULL, ADD COLUMN LVOFF18_COUNT12 INT NULL, ADD COLUMN LV10_COUNT8 INT NULL, ADD COLUMN LV10_COUNT12 INT NULL, ADD COLUMN LV18_SECONDS INT NULL,ADD COLUMN LESS_BREAK INT NULL,ADD COLUMN MORE_BREAK INT NULL";
    if (!sqlsrv_query($oconn,$query)) {
            ...
            if ($migrate_cur[2] != "" && $migrate_cur[3] != "") { 
                ...
                if (getRegister($txtMACAddress, 7) == "86") {
                    ...
                    if (getRegister($txtMACAddress, 7) == "14") {
                        ...
                        echo $query = "SELECT id, phone, idno FROM tuser WHERE " . $migrate_cur[0] . " = '" . $migrate_cur[1] . "' ORDER BY id";
                    }
                }
                if ($csv == "csv") {
                ...
                while ($cur = mysqli_fetch_row($result)) { 
                    ...
                    $breakmore = getCountSum($lconn, $where . " AND MoreBreak >'0' ", "SUM", "MoreBreak", $cur[0]);
                    $breakless = getCountSum($lconn, $where . " AND LessBreak >'0' ", "SUM", "LessBreak", $cur[0]);
                    ...
                    if ($rgresult = sqlsrv_query($oconn, $rgquery)) {
                        while ($rgcur = sqlsrv_fetch_array($rgresult,SQLSRV_FETCH_NUMERIC)) {
                            $regcal = $rgcur[0];
                        }
                    } else {
                        if (getRegister($txtMACAddress, 7) == "82" || getRegister($txtMACAddress, 7) == "103" || getRegister($txtMACAddress, 7) == "106" || getRegister($txtMACAddress, 7) == "107" || getRegister($txtMACAddress, 7) == "126" || getRegister($txtMACAddress, 7) == "57" || getRegister($txtMACAddress, 7) == "58" || getRegister($txtMACAddress, 7) == "59" || getRegister($txtMACAddress, 7) == "60" || getRegister($txtMACAddress, 7) == "117" || getRegister($txtMACAddress, 7) == "150" || getRegister($txtMACAddress, 7) == "3") {
                    ...
                    if (getRegister($txtMACAddress, 7) == "82" || getRegister($txtMACAddress, 7) == "103" || getRegister($txtMACAddress, 7) == "106" || getRegister($txtMACAddress, 7) == "107" || getRegister($txtMACAddress, 7) == "126" || getRegister($txtMACAddress, 7) == "57" || getRegister($txtMACAddress, 7) == "58" || getRegister($txtMACAddress, 7) == "59" || getRegister($txtMACAddress, 7) == "60" || getRegister($txtMACAddress, 7) == "117" || getRegister($txtMACAddress, 7) == "150" || getRegister($txtMACAddress, 7) == "3") {
                        ...
                        if (getRegister($txtMACAddress, 7) == "16") {
                            ...
                            $alterQuery = "ALTER TABLE dbo.tblEmpWorkHours_VIRDI ADD LESS_BREAK INT NULL,MORE_BREAK INT NULL";
                            sqlsrv_query($oconn, $alterQuery);
                            $payroll_query = "INSERT INTO dbo.tblEmpWorkHours_VIRDI (EMPPAYROLLNO, EMPNAME, FROMDATE, TODATE, MONTHNO, YEARNO, ISREGULARCASUAL, NORMAL_WORKING_SECONDS, NORMAL_WORKING_DAYS, ABSENT_SECONDS, ABSENT_DAYS, OT1_SECONDS, OT2_SECONDS, OT3_SECONDS, OT4_SECONDS, LV1_COUNT, LV2_COUNT, LV3_COUNT, LV4_COUNT, LV5_COUNT, LV6_COUNT, LV7_COUNT, LV8_COUNT, LV9_COUNT, LV10_COUNT, LV11_COUNT, LV12_COUNT, LV13_COUNT, LV14_COUNT, LV15_COUNT, LV16_COUNT, LV17_COUNT, LV18_COUNT, LV19_COUNT, LV20_COUNT, NIGHT_COUNT, ROTATE_COUNT, LATE_COUNT, EARLY_COUNT, TOTAL_GRACE, OT2_DAYS, OT3_DAYS, OT4_DAYS, NORMAL_WORKING_HOURS, OT1_HOURS, OT2_HOURS, OT3_HOURS, OT4_HOURS, WK_DAYS, WKSAT_DAYS, WKSUN_DAYS, ABSENT_SAT_DAYS, LATE_SECONDS, WK_BONUS_COUNT, EXTRA_HOURS,LESS_BREAK,MORE_BREAK) VALUES ('" . addZero($cur[0], $txtECodeLength) . "', '.', '" . displayParadoxDate($last_cur[0]) . " 00:00:00', '" . displayParadoxDate($txtLockDate) . " 00:00:00', " . $mm . ", " . $yyyy . ", " . $regcal . ", " . $normal . ", " . $ot_1 . ", " . ($late_sum + $early_sum) . ", " . $a_day . ", " . $ot1 . ", " . $ot2 . ", " . $ot3 . ", " . $ot4 . ", " . $v . ", " . $i . ", " . $b . ", " . $g . ", " . $y . ", " . $o . ", " . $r . ", " . $gr . ", " . $br . ", " . $pr . ", " . $mg . ", " . $tl . ", " . $aq . ", " . $sf . ", " . $ab . ", " . $gl . ", " . $vm . ", " . $sl . ", " . $mr . ", " . $pk . ", " . $night . ", " . $rotate . ", " . $late . ", " . $early . ", " . $grace . ", " . $ot_2 . ", " . $ot_3 . ", " . $ot_4 . ", " . round($normal / 3600, 2) . ", " . round($ot1 / 3600, 2) . ", " . round($ot2 / 3600, 2) . ", " . round($ot3 / 3600, 2) . ", " . round($ot4 / 3600, 2) . ", " . $WK_DAYS . ", " . $WKSAT_DAYS . ", " . $WKSUN_DAYS . ", " . $a_sat_day . ", " . $late_sum . ", " . $wk_bonus_count . ", " . $mcount . ",".round($breakless / 3600, 2).",".round($breakmore / 3600, 2).") ";
                        }
                    }
                    if (!sqlsrv_query($oconn, $payroll_query)) {
                        if ($csv != "csv") {
                            if (getRegister($txtMACAddress, 7) == "82" || getRegister($txtMACAddress, 7) == "103" || getRegister($txtMACAddress, 7) == "106" || getRegister($txtMACAddress, 7) == "107" || getRegister($txtMACAddress, 7) == "126" || getRegister($txtMACAddress, 7) == "57" || getRegister($txtMACAddress, 7) == "58" || getRegister($txtMACAddress, 7) == "59" || getRegister($txtMACAddress, 7) == "60" || getRegister($txtMACAddress, 7) == "117" || getRegister($txtMACAddress, 7) == "150" || getRegister($txtMACAddress, 7) == "3") {
                                ...
                                if (getRegister($txtMACAddress, 7) == "16") {
                                    ...
                                    if (getRegister($txtMACAddress, 7) == "168") {
                                        ...
                                        $payroll_query = "INSERT INTO Employee_WorkHour_VIRDI (TenantId, Cmp_Id, Emp_Id, Emp_Payroll_No, Emp_Name, FROMDATE, TODATE, MONTHNO, YEARNO, Employment_Type, NORMAL_WORKING_SECONDS, NORMAL_WORKING_DAYS, ABSENT_SECONDS, ABSENT_DAYS, OT1_SECONDS, OT2_SECONDS, OT3_SECONDS, OT4_SECONDS, LV1_COUNT, LV2_COUNT, LV3_COUNT, LV4_COUNT, LV5_COUNT, LV6_COUNT, LV7_COUNT, LV8_COUNT, LV9_COUNT, LV10_COUNT, LV11_COUNT, LV12_COUNT, LV13_COUNT, LV14_COUNT, LV15_COUNT, LV16_COUNT, LV17_COUNT, LV18_COUNT, LV19_COUNT, LV20_COUNT, NIGHT_COUNT, ROTATE_COUNT, LATE_COUNT, EARLY_COUNT, TOTAL_GRACE, OT2_DAYS, OT3_DAYS, OT4_DAYS, NORMAL_WORKING_HOURS, OT1_HOURS, OT2_HOURS, OT3_HOURS, OT4_HOURS, WK_DAYS, WKSAT_DAYS, WKSUN_DAYS, ABSENT_SAT_DAYS, LATE_SECONDS, WK_BONUS_COUNT, EXTRA_HOURS, ABSENT_SUN_DAYS) VALUES ('" . $co_code . "', '" . $co_code . "', '" . addZero($cur[0], $txtECodeLength) . "', '" . addZero($cur[0], $txtECodeLength) . "', '.', '" . displayParadoxDate($last_cur[0]) . " 00:00:00', '" . displayParadoxDate($txtLockDate) . " 00:00:00', " . $mm . ", " . $yyyy . ", " . $regcal . ", " . $normal . ", " . $ot_1 . ", " . ($late_sum + $early_sum) . ", " . $a_day . ", " . $ot1 . ", " . $ot2 . ", " . $ot3 . ", " . $ot4 . ", " . $v . ", " . $i . ", " . $b . ", " . $g . ", " . $y . ", " . $o . ", " . $r . ", " . $gr . ", " . $br . ", " . $pr . ", " . $mg . ", " . $tl . ", " . $aq . ", " . $sf . ", " . $ab . ", " . $gl . ", " . $vm . ", " . $sl . ", " . $mr . ", " . $pk . ", " . $night . ", " . $rotate . ", " . $late . ", " . $early . ", " . $grace . ", " . $ot_2 . ", " . $ot_3 . ", " . $ot_4 . ", " . round($normal / 3600, 2) . ", " . round($ot1 / 3600, 2) . ", " . round($ot2 / 3600, 2) . ", " . round($ot3 / 3600, 2) . ", " . round($ot4 / 3600, 2) . ", " . $WK_DAYS . ", " . $WKSAT_DAYS . ", " . $WKSUN_DAYS . ", " . $a_sat_day . ", " . $late_sum . ", " . $wk_bonus_count . ", " . $mcount . ", " . $a_sun_day . ",".round($breakless / 3600, 2).",".round($breakmore / 3600, 2).") ";
