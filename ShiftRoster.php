    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-12 d-flex no-block align-items-center">
                <h4 class="page-title">Shift Roster</h4>
...
print "<form name='frm1' id='frm1' method='post' onSubmit='return checkSearch()' action='ShiftRoster.php'><input type='hidden' name='act' value='searchRecord'>";
if ($prints != "yes") {
...
//        print "<form name='frm1' method='post' onSubmit='return checkSearch()' action='ShiftRoster.php'><input type='hidden' name='act' value='searchRecord'>";
            ?>
            <div class="row">
                <div class="col-3">
    <?php
    ...
    <?php
    if ($prints == "yes") {
        displayTextbox("lstDisplayFlag", "Display Flags: ", $lstDisplayFlag, $prints, 12, "", "");
    } else {
        print "<label class='form-label'>Display Flags</label><select name='lstDisplayFlag' class='form-control'><option selected value='" . $lstDisplayFlag . "'>" . $lstDisplayFlag . "</option> <option value='Yes'>Yes</option> <option value='No'>No</option> </select>";
    }
    ?>
                </div>
                <div class="col-2">
    ...
    if ($prints != "yes") {
        ...
        print "<center><input name='btSearch' type='submit' value='Search Record' class='btn btn-primary'>&nbsp;&nbsp;&nbsp;<input type='button' value='Upload Shift' onClick='openWindow()' class='btn btn-primary'></center>";
        print "</div>";
        print "</div>";
    }
    ?>
...
if ($act == "searchRecord") {
    ...
    if ($prints != "yes") {
        print "<td bgcolor='#F0F0F0' align='center'><font face='Verdana' size='2'><b>All</b></font></td>";
    } else {
        print "<td bgcolor='#F0F0F0' align='center'><font face='Verdana' size='2'><b>&nbsp;</b></font></td>";
    }
    for ($i = 0; $i < $dayCount; $i++) {
    ...
    if ($prints != "yes" && 0 < $count && stripos($userlevel, $current_module . "A") !== false && $_SESSION[$session_variable . "LockDate"] < insertDate($txtFrom) && $_SESSION[$session_variable . "LockDate"] < insertDate($txtTo)) {
        ...
        print "<div class='col-2'>";
        print "<label>Select Shift to be added to the checked boxes</label>";
        $query = "SELECT id, name from tgroup WHERE id > 1 ORDER BY name";
        displayList("lstSetShift", "", $lstSetShift, $prints, $conn, $query, "", "25%", "75%");
        print "";
        print "<input name='btSubmit' type='button' value='Save Changes' onClick='javascript:checkSubmit()' class='btn btn-primary'>";
        print "</div>";
    }
    print "</div>";
    print "<div class='row'>";
    if ($prints != "yes" && 0 < $count && stripos($userlevel, $current_module . "E") !== false && $_SESSION[$session_variable . "LockDate"] < insertDate($txtFrom) && $_SESSION[$session_variable . "LockDate"] < insertDate($txtTo)) {
        ...
        print "<div class='col-1'></div>";
        print "<div class='col-2'>";
//        print '<input type="hidden" name="lstSetFlag" id="lstSetFlag" />';
        print "<label>Select Shift to be edited to <br>(Click on the Shifts above to change it to the Below Selected Shift)</label>";
    ...
    if ($prints != "yes") {
        print "<center><br><input type='button' value='Print' onClick='checkPrint(0)' class='btn btn-primary'>&nbsp;&nbsp;<input type='button' value='Excel' onClick='checkPrint(1)' class='btn btn-primary'></center>";
    }
    print "</p>";
}
print "<input type='hidden' name='txtTot' value='" . $tot . "'> <input type='hidden' name='txtRowCount' value='" . $row_count . "'>";
print "</div></form>";
...
<script>
    function openWindow() {
        window.open('UploadShift.php', 'UploadPreFlag', 'width=800;height=600;resize=no;menubar=no;addressbar=no');
    }
</script>
