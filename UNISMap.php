...
if (!(checkSession($userlevel, $current_module) && $_SESSION[$session_variable . "VirdiLevel"] == "Classic")) {
    header("Location: " . $config["REDIRECT"] . "?url=UNISMap.php&message=Session Expired or Security Policy Violated");
}
$conn = openConnection();
...
<div class="page-breadcrumb">
                    <div class="row">
                        <div class="col-12 d-flex no-block align-items-center">
                            <h4 class="page-title">UNIS Map</h4>
...
//print "<html><title>UNIS Map</title><body><center>";
...
if ($act == "editRecord") {
    for ($i = 0; $i < $txhCounter; $i++) {
        ...
        $query = "UPDATE UNISMap SET DateFrom = '" . $_POST["txtFrom" . $i] . "', DateTo = '" . $_POST["txtTo" . $i] . "', MonthNo = '" . $_POST["txtMonthNo" . $i] . "' WHERE Col = '" . $_POST["txhCol" . $i] . "'  AND Val = '" . $_POST["txhVal" . $i] . "' ";
    ...
    header("Location: UNISMap.php?message=" . $message);
} else {
    if ($act == "deleteRecord") {
...
if ($prints != "yes") {
    ...
    print "<form name='frm1' method='post' action='UNISMap.php?act=editRecord'>";
    print "<tr><td><font face='Verdana' size='2'><b>Access Col</b></font></td><td><font face='Verdana' size='1'><b>&nbsp;</b></font></td><td><font face='Verdana' size='2'><b>tuser Col</b></font></td><td><font face='Verdana' size='2'><b>Master Table</b></font></td><td><font face='Verdana' size='2'><b>Name</b></font></td><td><font face='Verdana' size='1'><b>&nbsp;</b></font></td><td><font face='Verdana' size='2'><b>temploye Col</b></font></td><td><font face='Verdana' size='2'><b>Master Table</b></font></td><td><font face='Verdana' size='2'><b>Name</b></font></td><td><font face='Verdana' size='1'><b>&nbsp;</b></font></td><td><font face='Verdana' size='1'><b>&nbsp;</b></font></td></tr>";
    $query = "SELECT MapID, ACol, UCol, UMaster, UMasterName, ECol, MMaster, EMasterName FROM UNISMap ";
    $result = mysqli_query($iconn, $query);
    for ($i = 0; $cur = mysqli_fetch_row($result); $i++) {
        print "<tr><td><font face='Verdana' size='1'>" . $cur[0] . "</font><input type='hidden' name='txhCol" . $i . "' value='" . $cur[0] . "'></td><td><font face='Verdana' size='1'>" . $cur[1] . "</font><input type='hidden' name='txhVal" . $i . "' value='" . $cur[1] . "'></td><td><input name='txtFrom" . $i . "' id='txtFrom" . $i . "' value='" . $cur[2] . "' size='12'></td><td><input name='txtTo" . $i . "' id='txtTo" . $i . "' value='" . $cur[3] . "' size='12'></td><td><input name='txtMonthNo" . $i . "' id='txtMonthNo" . $i . "' value='" . $cur[4] . "' size='12'></td><td><a href='UNISMap.php?act=deleteRecord&txhCol=" . $cur[0] . "&txhVal=" . $cur[1] . "&txtFrom=" . $cur[2] . "&txtTo=" . $cur[3] . "'><font face='Verdana' size='2'><b>Delete</b></font></a></td></tr>";
    }
    if ($pay_result[0] == "") {
//        $query = "SELECT Col, Val, DateFrom, DateTo, MonthNo FROM UNISMap WHERE DateFrom = '' OR DateFrom = '0' OR DateFrom IS NULL ORDER BY Col, Val ";
        $query = "SELECT * FROM UNISMap";
    ...
    if (stripos($userlevel, $current_module . "E") !== false) {
        print "<p align='center'><input name='btSave' class='btn btn-primary' type='button' value='Save Changes' onClick='javascript:checkSubmit(" . $i . ")'></p>";
    }
