...
if ($message == "") {
    $message = "Early Out Report <br> (It is recommended that you DO NOT use a long Date Period)";
}
...
if ($prints != "yes") {
    ...
    if ($excel != "yes") {
        ...
        header("Content-Disposition: attachment; filename=ReportLIEOcount.xls");
...
if ($excel != "yes") {
    ...
    displayTextbox("ex3", "Early Depart Minute(s) <b>MORE</b> THAN: ", $ex3, $prints, 5, "30%", "25%");
    print "</tr></table></td>";
    print "</tr>";
    print "<tr>";
    print "<td colspan='2'><table width='100%' cellspacing='-1' cellpadding='-1' border='0'><tr>";
    displayTextbox("txtTo", "Date To <font size='1'>(DD/MM/YYYY)</font>: ", $txtTo, $prints, 12, "25%", "20%");
    displayTextbox("ex4", "Early Depart Minute(s) <b>LESS</b> THAN: ", $ex4, $prints, 5, "30%", "25%");
    print "</tr></table></td>";
    print "</tr>";
    if ($prints != "yes") {
    ...
    for ($date_count = insertDate($txtFrom); $date_count <= insertDate($txtTo); $date_count++) {
        if (checkdate(substr($date_count, 4, 2), substr($date_count, 6, 2), substr($date_count, 0, 4))) {
            if ($_SESSION[$session_variable . "ExitTerminal"] == "Yes") {
                $query = "SELECT tuser.id, tuser.name, tuser.dept, tuser.company, tgroup.name, tenter.e_date, tenter.e_time, tgate.name, tuser.idno, tuser.remark, tgroup.Close, tgroup.NightFlag, tuser.F1, tuser.F2, tuser.F3, tuser.F4, tuser.F5, tuser.F6, tuser.F7, tuser.F8, tuser.F9, tuser.F10, tgroup.Start, tgroup.GraceTo FROM tuser, tgroup, tenter, tgate WHERE tenter.e_group = tgroup.id AND tenter.e_id = tuser.id AND tenter.g_id = tgate.id AND tgroup.Close >= '0000' AND tuser.id > 0 AND tgate.exit = 0 " . $_SESSION[$session_variable . "DeptAccessQuery"] . " " . $_SESSION[$session_variable . "DivAccessQuery"] . "  AND tuser.id IN (SELECT DISTINCT(tuser.id) FROM tuser, tenter, tgate WHERE tenter.e_id = tuser.id AND tenter.g_id = tgate.id AND tgate.exit = 1) AND ((tenter.e_time < '240000' AND tgroup.NightFlag =0) OR (tenter.e_time < '" . $NightShiftMaxOutTime . "00' AND tgroup.NightFlag = 1)) ";
            } else {
                $query = "SELECT tuser.id, tuser.name, tuser.dept, tuser.company, tgroup.name, tenter.e_date, tenter.e_time, tgate.name, tuser.idno, tuser.remark, tgroup.Close, tgroup.NightFlag, tuser.F1, tuser.F2, tuser.F3, tuser.F4, tuser.F5, tuser.F6, tuser.F7, tuser.F8, tuser.F9, tuser.F10, tgroup.Start, tgroup.GraceTo FROM tuser, tgroup, tenter, tgate WHERE tenter.e_group = tgroup.id AND tenter.e_id = tuser.id AND tgroup.Close >= '0000' AND tuser.id > 0 AND tenter.g_id = tgate.id " . $_SESSION[$session_variable . "DeptAccessQuery"] . " " . $_SESSION[$session_variable . "DivAccessQuery"] . " AND ((tenter.e_time < '240000' AND tgroup.NightFlag =0) OR (tenter.e_time < '" . $NightShiftMaxOutTime . "00' AND tgroup.NightFlag = 1)) ";
            }
            if ($lstShift != "") {
            ...
            if (0 < mysqli_num_rows($result)) {
                while ($cur = mysqli_fetch_row($result)) {
                    if ($cur[0] == $last_id && $cur[5] == $last_date) {
                        ...
                        if ($data6 < getEarlyTime($date_count, $data10, $ex3) && getEarlyTime($date_count, $data10, $ex4) < $data6 && strlen($data5) == 8 && 0 < $ecount) {
                            if ($data3 == "") {
                                $data3 = "&nbsp;";
                            }
                            if ($data8 == "") {
                            ...
                            displayDate($data5);
                            $earlyMin = getEarlyMin($date_count, $data10, $data6);
                            $employeeId = addZero($data0, $_SESSION[$session_variable . "EmployeeCodeLength"]);
                            $employeeName = $data1;
                            $gen = $data8;
                            $dept = $data2;
                            $div = $data3;
                            ...
                            if (!isset($employeeEarlyMin[$employeeId])) {
                                $employeeEarlyMin[$employeeId] = [
                                    'name' => $employeeName,
                                    'gen' => $gen,
                                    'dept' => $dept,
                                    'div' => $div,
                            ...
                            $employeeEarlyMin[$employeeId]['total_early_min'] += ($earlyMin > 0) ? $earlyMin : 0;
//                            print "<tr><td><a title='ID'><input type='hidden' name='txh" . $count . "' value='" . $data0 . "'><font face='Verdana' size='1'>" . addZero($data0, $_SESSION[$session_variable . "EmployeeCodeLength"]) . "</font></a></td> <td><a title='Name'><font face='Verdana' size='1'>" . $data1 . "</font></a></td> <td><a title='" . $_SESSION[$session_variable . "IDColumnName"] . "'><font face='Verdana' size='1'>" . $data8 . "</font></a></td> <td><a title='Dept'><font face='Verdana' size='1'>" . $data2 . "</font></a></td> <td><a title='Div/Desg'><font face='Verdana' size='1'>" . $data3 . "</font></a></td> <td><a title='Rmk'><font face='Verdana' size='1'>" . $data9 . "</font></a></td> <td><a title='Shift'><font face='Verdana' size='1'>" . $data4 . "</font></a></td> <td><a title='Close'><font face='Verdana' size='1'>" . displayVirdiTime($data10 . "00") . "</font></a></td> <td><a title='Date'><font face='Verdana' size='1'>" . displayDate($data5) . "</font></a></td> <td><a title='Depart'><font face='Verdana' size='1'><b>" . displayVirdiTime($data6) . "</b></font></a></td> <td><a title='Early Minutes'><font face='Verdana' size='1'><b>" . getEarlyMin($date_count, $data10, $data6) . "</b></font></a></td> <td><a title='Terminal'><font face='Verdana' size='1'>" . $data7 . "</font></a></td></tr>";
            ...
            if (0 < mysqli_num_rows($result) && $data6 < getEarlyTime($date_count, $data10, $ex3) && 0 < $ecount) {
                ...
                addZero($data0, $_SESSION[$session_variable . "EmployeeCodeLength"]);
                displayVirdiTime($data10 . "00");
                displayVirdiTime($data22 . "00");
                displayDate($data5);
                displayVirdiTime($data6);
                $earlyMin = getEarlyMin($date_count, $data10, $data6);
                $lateMin = getLateMin($date_count, $data22, $data6);
                $employeeId = addZero($data0, $_SESSION[$session_variable . "EmployeeCodeLength"]);
                $employeeName = $data1;
                $gen = $data8;
                $dept = $data2;
...
//                        'total_late_min' => 0
                ...
                $employeeEarlyMin[$employeeId]['total_early_min'] += $earlyMin;
//                print "<tr><td><a title='ID'><input type='hidden' name='txh" . $count . "' value='" . $data0 . "'><font face='Verdana' size='1'>" . addZero($data0, $_SESSION[$session_variable . "EmployeeCodeLength"]) . "</font></a></td> <td><a title='Name'><font face='Verdana' size='1'>" . $data1 . "</font></a></td> <td><a title='" . $_SESSION[$session_variable . "IDColumnName"] . "'><font face='Verdana' size='1'>" . $data8 . "</font></a></td> <td><a title='Dept'><font face='Verdana' size='1'>" . $data2 . "</font></a></td> <td><a title='Div/Desg'><font face='Verdana' size='1'>" . $data3 . "</font></a></td> <td><a title='Rmk'><font face='Verdana' size='1'>" . $data9 . "</font></a></td> <td><a title='Shift'><font face='Verdana' size='1'>" . $data4 . "</font></a></td> <td><a title='Close'><font face='Verdana' size='1'>" . displayVirdiTime($data10 . "00") . "</font></a></td> <td><a title='Date'><font face='Verdana' size='1'>" . displayDate($data5) . "</font></a></td> <td><a title='Depart'><font face='Verdana' size='1'><b>" . displayVirdiTime($data6) . "</b></font></a></td> <td><a title='Early Minutes'><font face='Verdana' size='1'><b>" . getEarlyMin($date_count, $data10, $data6) . "</b></font></a></td> <td><a title='Terminal'><font face='Verdana' size='1'>" . $data7 . "</font></a></td></tr>";
                $count++;
            } 
        }
...
//                        echo "<pre>";print_R($cur[6]);
                        if (getLateTime($date_count, $cur[10], $ex3) < $cur[6] && 0 < getLateMin($date_count, $cur[10], $cur[6])) {
                            ...
                            displayVirdiTime($cur[11] . "00");
                            $employeeIds = addZero($cur[0], $_SESSION[$session_variable . "EmployeeCodeLength"]);
                            $employeeName = $cur[1];
                            $gen = $cur[8];
                            $dept = $cur[2];
                            $div = $cur[3];
                            $rmk = $cur[9];
                            // Add to employee total
                            if (!isset($employeeLateMin[$employeeIds])) {
                                $employeeLateMin[$employeeIds] = [
                                    'name' => $employeeName,
                                    'gen' => $gen,
                                    'dept' => $dept,
                                    'div' => $div,
...
//                                    'total_early_min' => 0,
                            ...
                            if ($lstLateness == "From Start Time") {
                                $lateMin = getLateMin($date_count, $cur[11], $cur[6]);
//                                print getLateMin($date_count, $cur[11], $cur[6]);
                            } else {
                                $lateMin = getLateMin($date_count, $cur[10], $cur[6]);
//                                print getLateMin($date_count, $cur[10], $cur[6]);
                            }
                            $employeeLateMin[$employeeIds]['total_late_min'] += ($lateMin > 0) ? $lateMin : 0;
                            $lateCount++;
                        }
                        $last_id = $cur[0];
                        $last_date = $cur[5];
...
echo "<tr>
        ...
        <th><font face='Verdana' size='2'>Division</font></th>
        <th><font face='Verdana' size='2'>Remark</font></th>
        <th><font face='Verdana' size='2'>Total Early Leave Minutes</font></th>
      </tr>";

foreach ($employeeEarlyMin as $employeeId => $stats) {
    echo "<tr>
            <td><font face='Verdana' size='1'>{$employeeId}</font></td>
            <td><font face='Verdana' size='1'>{$stats['name']}</font></td>
            <td><font face='Verdana' size='1'>{$stats['gen']}</font></td>
            <td><font face='Verdana' size='1'>{$stats['dept']}</font></td>
            <td><font face='Verdana' size='1'>{$stats['div']}</font></td>
            <td><font face='Verdana' size='1'>{$stats['rmk']}</font></td>
            <td><font face='Verdana' size='1'>{$stats['total_early_min']}</font></td>
          </tr>";
}
echo "</table>";
echo "<table border='1' cellpadding='1' bordercolor='#C0C0C0' cellspacing='-1' width='800'>";
echo "<tr>
        <th><font face='Verdana' size='2'>ID</font></th>
        <th><font face='Verdana' size='2'>Name</font></th>
        <th><font face='Verdana' size='2'>Gender</font></th>
        ...
        <th><font face='Verdana' size='2'>Division</font></th>
        <th><font face='Verdana' size='2'>Remark</font></th>
        <th><font face='Verdana' size='2'>Total LateIn Minutes</font></th>
      </tr>";
foreach ($employeeLateMin as $employeeIds => $statsData) {
    if($statsData['total_late_min'] > 0){
    echo "<tr>
            <td><font face='Verdana' size='1'>{$employeeIds}</font></td>
            <td><font face='Verdana' size='1'>{$statsData['name']}</font></td>
            <td><font face='Verdana' size='1'>{$statsData['gen']}</font></td>
            <td><font face='Verdana' size='1'>{$statsData['dept']}</font></td>
            <td><font face='Verdana' size='1'>{$statsData['div']}</font></td>
            <td><font face='Verdana' size='1'>{$statsData['rmk']}</font></td>
            <td><font face='Verdana' size='1'>{$statsData['total_late_min']}</font></td>
          </tr>";
    }
}
...
echo "</table>";
    if ($excel != "yes") {
        print "<br><p align='center'><font face='Verdana' size='1'>Total Record(s) Early Leave Displayed: <b>" . $count . "</b></font>";
        print "<br><p align='center'><font face='Verdana' size='1'>Total Record(s) LateIn Displayed: <b>" . $lateCount . "</b></font>";
    }
    if ($prints != "yes") {
