...
$result = selectData($conn, $query);
$query = "UPDATE tenter, tgate, tgroup SET tenter.e_group = " . $result[0] . " WHERE tenter.g_id = tgate.id AND tgate.name LIKE '%(D)%' AND tenter.p_flag = 0 AND tgroup.id = tenter.e_group AND tgroup.DNT = 'Yes' AND tenter.e_date > '" . $txtLockDate . "' ";
updateIData($iconn, $query, true);
$query = "SELECT id FROM tgroup WHERE DNT = 'Yes' AND NightFlag = 1 AND id > 2";
$result = selectData($conn, $query);
$query = "UPDATE tenter, tgate, tgroup SET tenter.e_group = " . $result[0] . " WHERE tenter.g_id = tgate.id AND tgate.name LIKE '%(N)%' AND tenter.p_flag = 0 AND tgroup.id = tenter.e_group AND tgroup.DNT = 'Yes' AND tenter.e_date > '" . $txtLockDate . "' ";
updateIData($iconn, $query, true);
        ...
        while ($sdcur1 = mysqli_fetch_row($result1)) {
            ...
            while ($cur2 = mysqli_fetch_row($result2)) {
                ...
                if ($cur2[4] < $cur2[3]) {
                    ...
                    $bout = mktime(substr($cur2[4], 0, 2), substr($cur2[4], 2, 2), substr($cur2[4], 4, 2), substr($cur2[1], 4, 2), substr($cur2[1], 6, 2), substr($cur2[1], 0, 4));
                    if (strlen($cur[5]) == 4) {
