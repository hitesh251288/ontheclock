...
class PHPMailer
{
    ...
    {
        if ($exceptions !== null) {
            $this->exceptions = (string) $exceptions;
        }
    }
    ...
    public function parseAddresses($addrstr, $useimap = true)
    {
        ...
        if ($useimap && function_exists("imap_rfc822_parse_adrlist")) {
            ...
            foreach ($list as $address) {
                if ($address->host != ".SYNTAX-ERROR." && $this->validateAddress($address->mailbox . "@" . $address->host)) {
    ...
    public static function validateAddress($address, $patternselect = null)
    {
        ...
        switch ($patternselect) {
            case "pcre8":
                return (string) preg_match("/^(?!(?>(?1)\"?(?>\\\\[ -~]|[^\"])\"?(?1)){255,})(?!(?>(?1)\"?(?>\\\\[ -~]|[^\"])\"?(?1)){65,}@)" . "((?>(?>(?>((?>(?>(?>\\x0D\\x0A)?[\\t ])+|(?>[\\t ]*\\x0D\\x0A)?[\\t ]+)?)(\\((?>(?2)" . "(?>[\\x01-\\x08\\x0B\\x0C\\x0E-'*-\\[\\]-\\x7F]|\\\\[\\x00-\\x7F]|(?3)))*(?2)\\)))+(?2))|(?2))?)" . "([!#-'*+\\/-9=?^-~-]+|\"(?>(?2)(?>[\\x01-\\x08\\x0B\\x0C\\x0E-!#-\\[\\]-\\x7F]|\\\\[\\x00-\\x7F]))*" . "(?2)\")(?>(?1)\\.(?1)(?4))*(?1)@(?!(?1)[a-z0-9-]{64,})(?1)(?>([a-z0-9](?>[a-z0-9-]*[a-z0-9])?)" . "(?>(?1)\\.(?!(?1)[a-z0-9-]{64,})(?1)(?5)){0,126}|\\[(?:(?>IPv6:(?>([a-f0-9]{1,4})(?>:(?6)){7}" . "|(?!(?:.*[a-f0-9][:\\]]){8,})((?6)(?>:(?6)){0,6})?::(?7)?))|(?>(?>IPv6:(?>(?6)(?>:(?6)){5}:" . "|(?!(?:.*[a-f0-9]:){6,})(?8)?::(?>((?6)(?>:(?6)){0,4}):)?))?(25[0-5]|2[0-4][0-9]|1[0-9]{2}" . "|[1-9]?[0-9])(?>\\.(?9)){3}))\\])(?1)\$/isD", $address);
            case "pcre":
                return (string) preg_match("/^(?!(?>\"?(?>\\\\[ -~]|[^\"])\"?){255,})(?!(?>\"?(?>\\\\[ -~]|[^\"])\"?){65,}@)(?>" . "[!#-'*+\\/-9=?^-~-]+|\"(?>(?>[\\x01-\\x08\\x0B\\x0C\\x0E-!#-\\[\\]-\\x7F]|\\\\[\\x00-\\xFF]))*\")" . "(?>\\.(?>[!#-'*+\\/-9=?^-~-]+|\"(?>(?>[\\x01-\\x08\\x0B\\x0C\\x0E-!#-\\[\\]-\\x7F]|\\\\[\\x00-\\xFF]))*\"))*" . "@(?>(?![a-z0-9-]{64,})(?>[a-z0-9](?>[a-z0-9-]*[a-z0-9])?)(?>\\.(?![a-z0-9-]{64,})" . "(?>[a-z0-9](?>[a-z0-9-]*[a-z0-9])?)){0,126}|\\[(?:(?>IPv6:(?>(?>[a-f0-9]{1,4})(?>:" . "[a-f0-9]{1,4}){7}|(?!(?:.*[a-f0-9][:\\]]){8,})(?>[a-f0-9]{1,4}(?>:[a-f0-9]{1,4}){0,6})?" . "::(?>[a-f0-9]{1,4}(?>:[a-f0-9]{1,4}){0,6})?))|(?>(?>IPv6:(?>[a-f0-9]{1,4}(?>:" . "[a-f0-9]{1,4}){5}:|(?!(?:.*[a-f0-9]:){6,})(?>[a-f0-9]{1,4}(?>:[a-f0-9]{1,4}){0,4})?" . "::(?>(?:[a-f0-9]{1,4}(?>:[a-f0-9]{1,4}){0,4}):)?))?(?>25[0-5]|2[0-4][0-9]|1[0-9]{2}" . "|[1-9]?[0-9])(?>\\.(?>25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])){3}))\\])\$/isD", $address);
            case "html5":
                return (string) preg_match("/^[a-zA-Z0-9.!#\$%&'*+\\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}" . "[a-zA-Z0-9])?(?:\\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*\$/sD", $address);
            case "noregex":
                return 3 <= strlen($address) && 1 <= strpos($address, "@") && strpos($address, "@") != strlen($address) - 1;
            case "php":
            default:
                return (string) filter_var($address, FILTER_VALIDATE_EMAIL);
        }
    }
    ...
    protected function mailSend($header, $body)
    {
        ...
        $to = implode(", ", $toArr);
        $params = null;
        if (!empty($this->Sender) && $this->validateAddress($this->Sender) && self::isshellsafe($this->Sender)) {
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if (("tls" === $secure || "ssl" === $secure) && !$sslext) {
            ...
            if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $options)) {
                try {
                    ...
                    if ($this->SMTPAuth && !$this->smtp->authenticate($this->Username, $this->Password, $this->AuthType, $this->Realm, $this->Workstation)) {
    ...
    {
        if (is_a($this->smtp, "SMTP") && $this->smtp->connected()) {
            $this->smtp->quit();
            $this->smtp->close();
        }
    }
    ...
    {
        return (string) preg_match("/[\\x80-\\xFF]/", $text);
    }
    ...
    public function msgHTML($message, $basedir = "", $advanced = false)
    {
        ...
        if (array_key_exists(2, $images)) {
            if (1 < strlen($basedir) && substr($basedir, 0 - 1) != "/") {
                $basedir .= "/";
            }
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (!empty($basedir) && strpos($url, "..") === false && substr($url, 0, 4) !== "cid:" && !preg_match("#^[a-z][a-z0-9+.-]*:?//#i", $url)) {
                    ...
                    $cid = md5($url) . "@phpmailer.0";
                    if (1 < strlen($directory) && substr($directory, 0 - 1) != "/") {
                        $directory .= "/";
                    }
                    if ($this->addEmbeddedImage($basedir . $directory . $filename, $cid, $filename, "base64", self::_mime_types((bool) self::mb_pathinfo($filename, PATHINFO_EXTENSION)))) {
    ...
    public function DKIM_Sign($signHeader)
    {
        ...
        if (0 <= version_compare(PHP_VERSION, "5.3.0") && in_array("sha256WithRSAEncryption", openssl_get_md_methods(true))) {
    ...
    {
        return (string) preg_match("/^(.{" . (self::MAX_LINE_LENGTH + 2) . ",})/m", $str);
    }
