...
$txtTo = $_POST["txtTo"];
$lstCompany = $_POST['txtCompany'];
$lstSex = $_POST['lstSex'];
$txtLocation = $_POST['txtLocation'];
//$lstPhone = $_POST['txtPhone'];
if ($txtFrom == "") {
    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-12 d-flex no-block align-items-center">
                <h4 class="page-title">Monthly Summary Report</h4>
...
print "<form name='frm1' id='frm1' method='post' onSubmit='return checkSearch()' action='EmployeeMaster.php'><input type='hidden' name='act' value='searchRecord'>";
if ($prints != "yes") {
//    print "<body>";
    ...
    if ($excel != "yes") {
        ...
        header("Content-Disposition: attachment; filename=EmployeeSetting.xls");
...
//            print "<tr><td>&nbsp;</td><td><font face='Verdana' size='1'><b>Selected Options</b></font></td></tr>";
    ...
    ?>
            ...
            <div class="row">
                <?php
                if (!($prints == "yes" && $lstLocation == "")) {
                    print "<div class='col-2'>";
                    $query = "SELECT distinct(phone), phone from tuser " . $_SESSION[$session_variable . "DeptAccessWhereQuery"] . " ORDER BY UPPER(phone)";
                    displaylist("lstLocation", "Location: ", $lstLocation, $prints, $conn, $query, "", "", "");
                    print "</div>";
//                    print "<div class='col-2'>";
//                    displaytextbox("$txtLocation", ": ", $_POST["txtLocation"], $prints, 25, "", "");
//                    print "</div>"; 
                }
                ?>
                <div class="col-2">
                <?php
                $query = "SELECT distinct(idno), idno from tuser " . $_SESSION[$session_variable . "DeptAccessWhereQuery"] . " AND idno<>'NULL' ORDER BY UPPER(idno)";
                displaylist("lstSex", "Gender: ", $lstSex, $prints, $conn, $query, "", "", "");
                ?>
                </div>
                    <?php
                    if ($prints != "yes") {
                        ...
                        print "</div>";
                        print "<div class='col-2'>";
                        print "<label class='form-label'>User Level:</label><select name='lstStatus' class='form-select select2 shadow-none'> <option selected value='" . $lstStatus . "'>" . $lstStatus . "</option> <option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option><option value='6'>6</option><option value='7'>7</option><option value='8'>8</option><option value='9'>9</option><option value='10'>10</option></select>";
                        print "</div>";
                        print "<div class='col-2'>";
...
//            displaySuperHeader($prints, $excel, $csv, $current_module, $userlevel, "Employee Settings", false, false);
            if ($act == "searchRecord") { //tuser.CAGID = CAG.CAGID AND 
                ...
                if ($lstMissingData != "") {
                    ...
                    if ($lstLocation != "") {
                        $query = $query . " AND tuser.phone = '" . $lstLocation . "'";
                    }
                    if ($lstSex != "") {
                        $query = $query . " AND tuser.idno = '" . $lstSex . "'";
                    }
                ...
                if ($csv != "yes") {
                    if ($prints == "yes") {
                        ...
                        print "<table border='1' cellpadding='1' bordercolor='#C0C0C0' cellspacing='-1' width='100%' class='table table-striped table-bordered dataTable'><thead><tr><td><font face='Verdana' size='2'><input type='checkbox' name='chkAll' id='chkAll' onClick='checkAll()'></font></td>";
                    }
                }
                ...
                while ($cur = mysqli_fetch_row($result)) {
                    ...
                    if ($csv != "yes") {
                        ...
                        if ($prints != "yes" && strpos($userlevel, $current_module . "A") !== false) {
                            ...
                            print "<td><input type='hidden' name='txh" . $count . "' value='" . $cur[0] . "'><a title='ID' name='" . $cur[0] . "' href='#" . $cur[0] . "' onClick='javascript:openWindow(" . $cur[0] * 419 . ")' style='text-decoration:underline;'><font face='Verdana' color='#000000' size='1'>" . addZero($cur[0], $_SESSION[$session_variable . "EmployeeCodeLength"]) . "</font></a></td>";
                        } else {
                            addZero($cur[0], $_SESSION[$session_variable . "EmployeeCodeLength"]);
                ...
                if ($prints != "yes" && $excel != "yes") {
                    if (strpos($userlevel, $current_module . "E") !== false) {
                        ...
                        displayList("lstSelectedDepartment", "Change the <b>Department</b> of the selected Record(s) to: ", $lstSelectedDepartment, $prints, $conn, $query, "", "", "");
                        print "</div>";
                        print "<div class='col-2'>";
                        $query = "SELECT DISTINCT(company), company from tuser ORDER BY company";
                        displayList("lstSelectedDivision", "Change the <b>" . $_SESSION[$session_variable . "DivColumnName"] . "</b> of the selected Record(s) to: ", $lstSelectedDivision, $prints, $conn, $query, "", "", "");
                        print "</div>";
                        print "<div class='col-2'>";
                        $query = "SELECT DISTINCT(idno), idno from tuser ORDER BY idno";
                        displayList("txtSelectedSNo", "Change the <b>" . $_SESSION[$session_variable . "IDColumnName"] . "</b> of the selected Record(s) to: ", $txtSelectedSNo, $prints, $conn, $query, "", "", "");
                        print "</div>";
                        print "<div class='col-2'>";
                        $query = "SELECT DISTINCT(Remark), Remark from tuser ORDER BY Remark";
                        displayList("txtSelectedRemark", "Change the <b>" . $_SESSION[$session_variable . "RemarkColumnName"] . "</b> of the selected Record(s) to: ", $txtSelectedRemark, $prints, $conn, $query, "", "", "");
                        print "</div>";
                        print "<div class='col-2'>";
                        $query = "SELECT DISTINCT(Phone), Phone from tuser ORDER BY Phone";
                        displayList("txtSelectedPhone", "Change the <b>" . $_SESSION[$session_variable . "PhoneColumnName"] . "</b> of the selected Record(s) to: ", $txtSelectedPhone, $prints, $conn, $query, "", "", "");
                        print "</div>";
                        for ($i = 0; $i < 10; $i++) {
                            print "<div class='col-2'>";
                            if ($_SESSION[$session_variable . "F" . ($i + 1)] != "") {
                                ...
                                displayList("lstSelectedF" . ($i + 1), "Change the <b>" . $_SESSION[$session_variable . "F" . ($i + 1)] . "</b> of the selected Record(s) to: ", $_POST["lstSelectedF" . ($i + 1)], $prints, $conn, $query, "", "", "");
                            } else {
                        ...
                        print "<div class='row'>";
                        print "<div class='col-2'>";
                        print "<label class='form-label'>Change the <b>Level</b> of the selected Record(s) to:</label><select name='lstSelectedLevel' class='form-control'> <option selected value='" . $lstStatus . "'>" . $lstStatus . "</option>";
                        ...
                        print "<div class='col-2'>";
                        $query = "SELECT CAGID, Name from CAG ORDER BY Name";
                        displayList("lstSelectContractAccess", "Change the <b>Contract Access Group</b> of the selected Record(s) to: ", $lstContractAccess, $prints, $conn, $query, "", "", "");
                        print "</div>";
                        print "</div>";
                        print "<div class='row'>";
                        print "<div class='col-12 mb-3'>";
                        print "<center><br><input name='btSubmit' type='button' class='btn btn-primary' value='Save Changes' onClick='saveChanges()'></center>";
                        print "</div>";
                        print "</div>";
                        print "<div class='row'>";
                        print "<div class='col-2'></div>";
                        print "<div class='col-2'>";
                        displayEmployeeStatus($conn, "lstSelectedStatus", $lstSelectedStatus, "", "");
                        print "</div>";
                        print "<div class='col-4'>";
                        displayTextbox("txtSelectedEndDate", "End Date for Employees to be De-Activated <font size='1'><font size='1'>(DD/MM/YYYY)</font></font>: ", $txtSelectedEndDate, $prints, 15, "", "");
                        print "</div>";
                        print "</div>";
                        print "<div class='row'>";
//                        print "<div class='col-2'></div>";
                        ...
                        if (strpos($userlevel, $current_module . "D") !== false) {
                            print "<td align='right'><input type='button' class='btn btn-primary' value='De-Activate Selected Record(s)' onClick='deActivate()'></td>";
                        } else {
                            print "<td>&nbsp;</td>";
                        }
                        print "</div>";
                        print "<div class='col-3'>";
                        if (strpos($userlevel, $current_module . "D") !== false) {
                            print "<td><input type='button' class='btn btn-primary' value='Activate Selected Record(s)' onClick='activate()'></td>";
                        } else {
                            print "<td>&nbsp;</td>";
                        }
                        print "</div>";
                        if (strpos($userlevel, $current_module . "D") !== false && strpos($userlevel, "11D") !== false) {
                            print "<div class='col-3'>";
                            print "<input type='button' class='btn btn-primary' value='Emigrate Selected Record(s)' onClick='emigrate()'>";
                            print "</div>";
                            print "<div class='col-3'>";
                            print "<input type='button' class='btn btn-primary' value='Migrate Selected Record(s)' onClick='migrate()'>";
                            print "</div>";
                        }
                        print "</div>";
                    }
                    print "<br><p align='center'><input type='button' class='btn btn-primary' value='Print Report' onClick='checkPrint(0)'>&nbsp;&nbsp;<input type='button' class='btn btn-primary' value='Excel' onClick='checkPrint(1)'>&nbsp;&nbsp;<input type='button' class='btn btn-primary' value='CSV' onClick='checkPrint(3)' id='export-btn'>";
                    print "</p>";
                }
            }
            if ($csv != "yes") {
...
//        }
        function checkAll() { 
            ...
            // Verify that txtCount exists and has a value
            var z = x.txtCount ? x.txtCount.value : 0; // Total count of checkboxes
        ...
        document.getElementById('export-btn').addEventListener('click', function () {  
            ...
            link.setAttribute('download', 'EmployeeSetting.csv');  
