    ...
    if ($act == "deleteSelected") {
        ...
        if ($act == "saveChanges") {
                    ...
                    if ($lstEditShift != "" && $_POST["chk" . $i] == "on") {
                        ...
                        if ($lstEditTerminal != "" && $_POST["chk" . $i] == "on") {
                            ...
                            if (updateIData($iconn, $query, true)) {
                                ...
                                $enter_time = $_POST["txtTime" . $i] ?? '';

                                if ($enter_time !== '') {
                                    // Normalize: if "1900" -> "190000", if "730" -> "073000"
                                    $enter_time = str_pad(substr($enter_time, 0, 6), 6, '0', STR_PAD_RIGHT);
                                }
                                $query = "
                                        INSERT INTO tenter 
                                                (e_date, e_time, g_id, e_id, e_name, e_idno, e_group, e_user, e_mode, e_type, e_result, e_etc, p_flag, e_uptime, e_upmode)
                                        VALUES
                                                ('$e_date', '$enter_time', $g_id, $e_id, '$e_name', '$e_idno', $e_group, $e_user, $e_mode, $e_type, $e_result, '$e_etc', $p_flag, '$e_uptime', '$e_upmode')
                        ...
                        updateIData($jconn, $query, true);
                            ...
                            $query = "
                                INSERT INTO AlterLog (Username, ed, DateFrom, TimeFrom, DateTo, TimeTo, TransactDate)
                                VALUES (
                                    '$username',
                                    $ed,
                                    '$dateFrom',
                                    '" . substr(str_pad($_POST["txhTime" . $i], 6, '0', STR_PAD_RIGHT), 0, 6) . "',
                                    '$dateTo',
                                    '" . substr(str_pad($_POST["txtTime" . $i], 6, '0', STR_PAD_RIGHT), 0, 6) . "',
                                    " . insertToday() . "
                                )
                            ";
            ...
            if ($act == "fixSingleClocking") {
                $e_date  = !empty($sub_date) ? $sub_date : date("Ymd"); 
//                $sub_date = "";
                $sub_id = "";
                $sub_time = "";
                $sub_gate = "";
                ...
                $sub_bfrom = "";
                $sub_bto = "";
                $sub_query = "SELECT tenter.e_date, tenter.e_time, tenter.g_id, tenter.e_id, tgroup.Start, tgroup.Close, tgroup.WorkMin, tgroup.NightFlag, tenter.e_group, tgroup.ScheduleID , tgate.name, tgroup.BreakFrom, tgroup.BreakTo FROM tenter, tuser, tgroup, tgate WHERE tenter.e_id = tuser.id AND tenter.e_group = tgroup.id AND tenter.g_id = tgate.id AND tgate.exit = 0 AND tgroup.ShiftTypeID = 1 AND tgroup.NightFlag = 0 AND tenter.p_flag = 0 " . $_SESSION[$session_variable . "DeptAccessQuery"] . " " . $_SESSION[$session_variable . "DivAccessQuery"] . " AND tenter.e_date >= '" . insertDate($txtFrom) . "' AND tenter.e_date <= '" . insertDate($txtTo) . "' AND tenter.e_date < '" . insertToday() . "'";
                if ($lstShift != "") {
                ...
                for ($sub_result = mysqli_query($conn, $sub_query); $sub_cur = mysqli_fetch_row($sub_result); $counter++) {
                    if ($counter == 1 && ($sub_date != $sub_cur[0] || $sub_id != $sub_cur[3])) {
                        ...
                        if ($sub_night_flag == 0 && $halfTime <= $sub_time) {
                            ...
                            updateIData($iconn, $query, true);
                            $e_date  = !empty($sub_date) ? $sub_date : date("Ymd");   // fallback to today
                            $e_time  = !empty($sub_start) ? $sub_start . "00" : date("His");
                            $g_id    = (int)$sub_gate;
                            $e_id    = (int)$sub_id;
                            $e_group = (int)$sub_group;
//                            $query = "INSERT INTO tenter (e_date, e_time, g_id, e_id, e_group, e_user, e_mode, e_type, e_result, e_etc) VALUES ('" . $sub_date . "', '" . $sub_start . "00', " . $sub_gate . ", " . $sub_id . ", " . $sub_group . ", '0', '3', '3', '0', 'P')";
                            $query = "INSERT INTO tenter 
                                    ...
                                    VALUES 
                                        ('$e_date', '$e_time', $g_id, $e_id, $e_group, '0', '3', '3', '0', 'P')
                                    ON DUPLICATE KEY UPDATE
                                        e_group = VALUES(e_group),
                                        e_user  = VALUES(e_user),
                                        e_mode  = VALUES(e_mode),
...
//                            echo "DEBUG → punchType=$punchType punchTime=$punchTime<br>";die;
                ...
                if ($counter == 1) {
                    ...
                    if ($sub_night_flag == 0 && $halfTime <= $sub_time || $sub_night_flag == 1 && $sub_time <= $nightShiftMaxOutTime) {
                        ...
                        $query = "UPDATE tenter SET p_flag = 0, e_etc = 'P' WHERE p_flag = '1' AND e_date = '" . $sub_date . "' AND e_time = '" . $sub_start . "00' AND g_id = " . $sub_gate . " AND e_id =  " . $sub_id;
                        updateIData($iconn, $query, true);
                        $query = "INSERT INTO tenter (e_date, e_time, g_id, e_id, e_group, e_user, e_mode, e_type, e_result, e_etc) VALUES ('" . $sub_date . "', '" . $sub_start . "00', " . $sub_gate . ", " . $sub_id . ", " . $sub_group . ", '0', '3', '3', '0', 'P') ON DUPLICATE KEY UPDATE e_group=VALUES(e_group), e_user=VALUES(e_user), e_mode=VALUES(e_mode), e_type=VALUES(e_type), e_result=VALUES(e_result), e_etc=VALUES(e_etc)";
                        updateIData($jconn, $query, true);
                        $query = "SELECT ed FROM tenter WHERE e_date = '" . $sub_date . "' AND e_time = '" . $sub_start . "00' AND g_id = " . $sub_gate . " AND e_id = " . $sub_id . " AND e_group = " . $sub_group;
                        $max = selectData($conn, $query);
						$edValue = !empty($max[0]) ? $max[0] : 0;
                        $query = "INSERT INTO AlterLog (Username, ed, DateTo, TimeTo, GateTo, TransactDate, ShiftTo) VALUES ('" . $username . "', " . $edValue . ", '" . $sub_date . "', '" . $sub_start . "00', " . $sub_gate . ", " . insertToday() . ", " . $sub_group . ")";
                        updateIData($kconn, $query, true);
                        $timeHHMM = substr($sub_time, 0, 4); // e.g. 1800
                        if ($timeHHMM == $sub_close) {
                            // Only OUT punch present → add missing IN
                        ...
                        if (($sub_night_flag == 0 && $sub_time < $halfTime || $sub_night_flag == 1 && $nightShiftMaxOutTime < $sub_time) && $sub_date < insertToday()) {
                            ...
                            updateIData($iconn, $query, true);
                            $query = "INSERT IGNORE INTO tenter (e_date, e_time, g_id, e_id, e_group, e_user, e_mode, e_type, e_result, e_etc) VALUES ('" . $sub_date . "', '" . $sub_close . "00', " . $sub_gate . ", " . $sub_id . ", " . $sub_group . ", '0', '3', '3', '0', 'P')";
                            updateIData($jconn, $query, true);
                            $query = "SELECT ed FROM tenter WHERE e_date = '" . $sub_date . "' AND e_time = '" . $sub_close . "00' AND g_id = " . $sub_gate . " AND e_id = " . $sub_id . " AND e_group = " . $sub_group;
                            $max = selectData($conn, $query);
...
//                            $query = "INSERT INTO tenter (e_date, e_time, g_id, e_id, e_group, e_user, e_mode, e_type, e_result, e_etc) VALUES ('" . $enter_date . "', '" . $enter_time . "00', " . $sub_gate . ", " . $sub_id . ", " . $sub_group . ", '0', '3', '3', '0', 'P')";
                            ...
                            if (!empty($enter_time) && strlen($enter_time) < 6) {
                                $enter_time = str_pad($enter_time, 6, '0', STR_PAD_RIGHT);
                            }
...
//    print "<center>";
    ...
    if ($act == "searchRecord") {
        $divAccess = $_SESSION[$session_variable . "DivAccessQuery"] ?? "";
        $deptAccess = $_SESSION[$session_variable . "DeptAccessQuery"] ?? "";
        $query = "SELECT tuser.id, tuser.name, tuser.dept, tuser.company, tgroup.name, tenter.e_date, tenter.e_time, tgate.name, tenter.ed, tuser.idno, tuser.remark, tenter.e_group, tenter.g_id FROM tuser, tgroup, tenter, tgate WHERE tenter.e_group = tgroup.id AND tenter.e_id = tuser.id AND tenter.g_id = tgate.id AND tenter.p_flag = 0 $deptAccess $divAccess";
        if ($lstShift != "") {
            $query = $query . " AND tgroup.id = " . $lstShift;
        }
        $lstGroup = isset($_POST["lstGroup"]) ? $_POST["lstGroup"] : "";
