...
if (checkMAC($conn) == true && noTASoftware("", $main_result[1]) == false && 2 < strlen($main_result[2]) && 2 < strlen($main_result[3])) {
    ...
    while ($user_cur = mysqli_fetch_row($user_result)) {
        $file_name = "mailer\\Proxy-Mailer-" . $user_cur[0] . "-" . insertToday() . "" . getNow() . ".xls";
        $dept_div_query = mailerDeptDiv($conn, $iconn, $jconn, $user_cur[0], $user_cur[2]);
        $txtFrom = substr(insertToday(), 0, 6) . "01";
        $query = "SELECT COUNT(AttendanceMaster.AttendanceID), tuser.id, tuser.name, tuser.dept, tuser.company, tuser.idno, tuser.remark FROM tuser, AttendanceMaster, tgroup WHERE AttendanceMaster.group_id = tgroup.id AND AttendanceMaster.EmployeeID = tuser.id AND AttendanceMaster.ADate >= '" . $txtFrom . "' AND AttendanceMaster.ADate < '" . insertToday() . "' AND AttendanceMaster.Flag = 'Proxy' " . $dept_div_query . " GROUP BY tuser.id, tuser.name, tuser.dept, tuser.company, tuser.idno, tuser.remark ORDER BY COUNT(AttendanceMaster.AttendanceID) DESC";
        $counter = 0;
        $last_id = "";
        $last_date = "";
        ...
        if (0 < mysqli_num_rows($result)) {
            ...
            while ($cur = mysqli_fetch_row($result)) {
                ...
                $data = "<tr><td><font face='Verdana' size='1'>" . displayDate($txtFrom) . "</font></td><td><font face='Verdana' size='1'>" . displayDate(getLastDay(insertToday(), 1)) . "</font></td><td><font face='Verdana' size='1'>" . addZero($cur[1], $main_result[7]) . "</font></td> <td><font face='Verdana' size='1'>" . $cur[2] . "</font></td> <td><font face='Verdana' size='1'>" . $cur[5] . "</font></td> <td><font face='Verdana' size='1'>" . $cur[3] . "</font></td> <td><font face='Verdana' size='1'>" . $cur[4] . "</font></td> <td><font face='Verdana' size='1'>" . $cur[6] . "</font></td> <td><font face='Verdana' size='1'><b>" . $cur[0] . "</b></font></td></tr>";
                fwrite($handle, $data);
                $last_id = $cur[1];
                $counter++;
            }
            $data = "</table>";
            $mail_query = "SELECT MailerText FROM MailerText WHERE MailerType = 'ProxyMailer'";
            ...
            fwrite($handle, $data);
            fclose($handle);
            if (sendMail("Proxy Mailer: Total Record(s) - " . $counter, str_replace("\r", "<br>", $mail_text), $mail_text, $file_name, "", "Virdi Admin", $u_email, $u_name, "", "", "", "")) {
                $count++;
            }
    ...
    print "Proxy Mailer Process Executed Successfully";
} else {
    print "Proxy Mailer COULD NOT be Executed";
}
