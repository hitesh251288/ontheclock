...
<div class="page-breadcrumb">
                    <div class="row">
                        <div class="col-12 d-flex no-block align-items-center">
                            <h4 class="page-title">Attendance Snap Shot Report</h4>
                            <div class="ms-auto text-end">
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item"><a href="#">Home</a></li>
                                        <li class="breadcrumb-item active" aria-current="page">
                                            Attendance Snap Shot Report
                                        </li>
                                    </ol>
                                </nav>
                            </div>
        ...
        if ($lstReportType == "Custom3") {
            ...
            while ($cur = mysqli_fetch_row($result)) {
                if ($eid != $cur[0]) {
                    ...
                    if ($csv != "yes") {
                        addZero($cur[0], $_SESSION[$session_variable . "EmployeeCodeLength"]);
                        print "<td><font face='Verdana' size='1' color='#000000'>" . addZero($cur[0], $_SESSION[$session_variable . "EmployeeCodeLength"]) . "</font></a></td>";
                        ...
                        if (strpos($ras, "-TLD-") !== false && $lstReportType != "Custom7") {
                            if ($csv != "yes") {
                                if ($lstReportType == "Custom6") {
                                    ...
                                    $adjusted_late = ($lateness_absence != 0) ? round($late_days / $lateness_absence) : 0;
//                                    round($late_days / $lateness_absence);
                                    if($lstReportType != "Custom9"){
                                        print "<td bgcolor='#F0F0F0'><a title='Total Days Present (Excluding the Flags SET be Ignored in Global Settings)'><font face='Verdana' size='1'>" . ($wkdn + $pxyn + $flgn + $satn + $sunn + $wkdo + $pxyo + $flgo + $sato + $suno - $tld - $adjusted_late) . "</font></a></td>";
                                    }
                                }
                            } else {
                                if ($lstReportType == "Custom6") {
                            ...
                            if ($lstReportType != "Custom2") {
                                $adjusted_late = ($lateness_absence != 0) ? round($late_days / $lateness_absence) : 0;
                                $this_record = $dayCount - $reg_day - ($wkdn + $pxyn + $flgn + $satn + $sunn + $wkdo + $pxyo + $flgo + $sato + $suno) + $adjusted_late;
                                if ($this_record < 0) {
                                    $this_record = 0;
                                }
                                if ($csv != "yes") {
                            ...
                            if (0 < $sub_result[0]) {
                                ...
                                if ($global_saturday == 0) {
                                    $adjusted_late = ($lateness_absence != 0) ? round($late_days / $lateness_absence) : 0;
                                    $this_record = $dayCount - $reg_day - $sunCount - ($wkdn + $pxyn + $flgn + $satn + $wkdo + $pxyo + $flgo + $sato - $tld) + $adjusted_late;
                                } else {
                                    ...
                                    if ($lateness_absence != 0) {
                                        $this_record = $dayCount - $reg_day + $h_satabo + $h_satabn 
                                            - $sunCount - ($wkdn + $pxyn + $flgn + $satn + $wkdo + $pxyo + $flgo + $sato - $tld)
                                            + round($late_days / $lateness_absence);
                                    } else {
                                        $this_record = $dayCount - $reg_day + $h_satabo + $h_satabn 
                                            - $sunCount - ($wkdn + $pxyn + $flgn + $satn + $wkdo + $pxyo + $flgo + $sato - $tld);
...
//                                        }
                                ...
                                $adjusted_late = ($lateness_absence != 0) ? round($late_days / $lateness_absence) : 0;
                                $this_record = $dayCount - $reg_day + $h_sunabo + $h_sunabn + $h_satabo + $h_satabn - $sunCount - $satCount - ($wkdn + $pxyn + $flgn + $wkdo + $pxyo + $flgo - $tld) + $adjusted_late;
                                if ($this_record < 0) {
                                    $this_record = 0;
                                }
                                if ($csv != "yes") {
...
//                            round($late_days / $lateness_absence);
                    ...
                    if (0 < $sub_result[0]) {
                        ...
                        if ($global_saturday == 0) {
                            ...
                            if ($lateness_absence != 0) {
                                $late_ratio = round($late_days / $lateness_absence);
                            } else {
                                $late_ratio = 0; // or another default value if needed
                            }
                            $this_record = $dayCount - $reg_day + $h_satabo + $h_satabn - $sunCount - ($wkdn + $pxyn + $flgn + $satn + $wkdo + $pxyo + $flgo + $sato - $tld) + $late_ratio;
                            //$this_record = $dayCount - $reg_day + $h_satabo + $h_satabn - $sunCount - ($wkdn + $pxyn + $flgn + $satn + $wkdo + $pxyo + $flgo + $sato - $tld) + round($late_days / $lateness_absence);
                        }
                        if ($this_record < 0) {
                            $this_record = 0;
...
//                            round($late_days / $lateness_absence);
                            if($lstReportType != "Custom8" && $lstReportType != "Custom9"){
                                print "<td bgcolor='#F0F0F0'><a title='Total Weekdays in the selected Period'><font face='Verdana' size='1'>" . ($wkdn + $pxyn + $flgn + $satn + $sunn + $wkdo + $pxyo + $flgo + $sato + $suno - $tld - $adjusted_late + $no_abdays) . "</font></a></td>";
                            }
                        }
                    } else {
                        if ($lstReportType == "Custom6") {
...
//                    }
                ...
                if (strpos($ras, "-SAT-") !== false) {
                    $sub_query = "SELECT COUNT(AOvertime) FROM AttendanceMaster WHERE EmployeeID = " . $cur[0] . " AND ADate >= " . insertDate($txtFrom) . " AND ADate <= " . insertDate($txtTo) . " AND Day = OT1 AND AOvertime > 0 ";
                    $sub_result = selectData($conn, $sub_query);
                    if ($csv != "yes") {
                        print "<td><a title='Saturday'><font face='Verdana' size='1'>".$sub_result[0]."</font></a></td>";
                    } else {
                        print "0;";
                    }
                }
