...
if (!checkSession($userlevel, $current_module)) {
    header("Location: " . $config["REDIRECT"] . "?url=HodAssignManager.php&message=Session Expired or Security Policy Violated");
}
//$conn = openConnection();
...
if($designationRow['F8'] == 'Admin'){
    $userQuery = "Select id,name from tuser where F8 = 'HOD'";
    ...
    $casualQuery = "SELECT t.id,t.name,h.parent_id,h.id as reportingid,s.name as hod FROM tuser t LEFT JOIN hodassign h ON h.child_id=t.id LEFT JOIN tuser s ON h.parent_id=s.id where t.F8 = 'PM'";
    $casualResult = mysqli_query($conn, $casualQuery);
    $casualData = mysqli_fetch_all($casualResult, MYSQLI_ASSOC);
}else{
    $userQuery = "Select id,name from tuser where F8 = 'HOD' AND dept='$dept'";
    //$userQuery = "SELECT employee_id,fullname FROM onboardrequest where grade LIKE 'S%' AND status_reg= '1'";
    $userResult = mysqli_query($conn, $userQuery);
    $userData = mysqli_fetch_all($userResult, MYSQLI_ASSOC);
    ...
    //$casualQuery = "SELECT id,name FROM tuser t where id LIKE '10%' AND remark != 'SENIOR STAFF'";
    //$casualQuery = "SELECT employee_id,fullname FROM onboardrequest where grade NOT LIKE 'S%' AND status_reg= '1'";
    $casualQuery = "SELECT t.id,t.name,h.parent_id,h.id as reportingid,s.name as hod FROM tuser t LEFT JOIN hodassign h ON h.child_id=t.id LEFT JOIN tuser s ON h.parent_id=s.id where t.F8 = 'PM' AND t.dept='$dept'";
    $casualResult = mysqli_query($conn, $casualQuery);
    $casualData = mysqli_fetch_all($casualResult, MYSQLI_ASSOC);
}
...
$hireQuery = "select parent_id, child_id from hodassign";
...
<form method="post" action="HodAssignManager.php">
    <table width="800" border="1" cellpadding="1" bordercolor="#C0C0C0" cellspacing="-1">
        <tbody>
            <tr>
                <td>&nbsp;</td>
                <td><font face="Verdana" size="1"><b>HOD List From Below</b></font></td>
            </tr>
            <tr>
                <td align="right" width="20%"><font size="2" face="Verdana">HOD : </font></td>
                <td width="80%">
        ...
        <tbody> 
            ...
            foreach ($casualData as $casualAll) { 
                ...
                if(isset($casualAll['parent_id'])){ 
                    $unassign = "<a href='HodAssignManager.php?id=" . $casualAll['reportingid'] . "' class='btn btn-primary' style='text-decoration:none;'>Unassign</a>";
                }else{
                    $unassign = "";
                }
                ?>
                <tr>
                    ...
                    <td><font face="Verdana" size="1"><?php echo $casualAll['hod']; ?></font></td>
    ...
    <br>
    <input type="submit" name="submit" value="Assign Manager"/>
</form>
...
if (isset($_POST['submit'])) {
    ...
    for ($i = 0; $i < count($childID); $i++) {
        $reportQuery = "Insert into hodassign(parent_id,child_id,assign_by)values($parentID,$childID[$i],'$assign')";
    ...
    header('Location:HodAssignManager.php?status=1&status=' . $repVar);
}

if(isset($_GET['id'])){
    $id = $_GET['id'];
    $query = "DELETE FROM hodassign where id=".$id;
    ...
    header('Location:HodAssignManager.php?message=' . $keyVar);
}
