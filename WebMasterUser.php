...
if (getRegister($txtMACAddress, 7) == "62") {
    $txtDBIP = $txtDBIP . ",1434\\SQLEXPRESS";
}
$co_code = 1;
...
for ($iii = 0; $iii < $main_count; $iii++) {
                            ...
                            if (getRegister($txtMACAddress, 7) == "53") {
                                ...
                                if (getRegister($txtMACAddress, 7) == "82") {
                                    ...
                                    if (getRegister($txtMACAddress, 7) == "103") {
                                        ...
                                        if (getRegister($txtMACAddress, 7) == "117") {
                                            $co_code = 11;
                                        } else {
                                            if (getRegister($txtMACAddress, 7) == "126") {
                                                $co_code = 9;
                                            }
                                        }
                                    }
    ...
    if ($oconn != "" && $main_result[1] != "" && checkMAC($conn) == true && $main_result[1] != "No Synchronization") {
        if ($main_result[1] == "Payroll DB") {
                ...
                if ($main_result[13] == "PayMaster") {
                    ...
                    if ($main_result[13] == "WebPayMaster") {
                        for ($synch = 4; $synch <= count($virdi); $synch++) {
                            for ($arr = 0; $arr < count($pay_id); $arr++) {
                                echo "\n\r" . $pay_map[$arr] . " - " . $main_result[$synch];
            ...
            $emp_code = 0;
            if ($main_result[13] == "WebPayMaster") {
                $query = "SELECT MAX(Emp_Id) FROM " . $main_result[0];
            ...
            if (getRegister($txtMACAddress, 7) == "86") {
                ...
                if (getRegister($txtMACAddress, 7) == "82" || getRegister($txtMACAddress, 7) == "103" || getRegister($txtMACAddress, 7) == "106" || getRegister($txtMACAddress, 7) == "107" || getRegister($txtMACAddress, 7) == "126" || getRegister($txtMACAddress, 7) == "57" || getRegister($txtMACAddress, 7) == "58" || getRegister($txtMACAddress, 7) == "59" || getRegister($txtMACAddress, 7) == "60" || getRegister($txtMACAddress, 7) == "117") {
            ...
            while ($cur = mysqli_fetch_row($result)) {
                ...
                if (is_numeric($payroll_cur[0]) == "") {
                    if ($main_result[13] == "WebPayMaster") {
                        $payroll_insert_query = "SET IDENTITY_INSERT tblEmployee ON";
                        mssql_query($payroll_insert_query, $oconn);
                        $payroll_insert_query = "INSERT INTO " . $main_result[0] . " (Emp_Code, Emp_Id, TenantID, Emp_First_Name, Emp_Middle_Name, Emp_Last_Name, Emp_Gender, Employment_Type, Emp_Children, Emp_Dependents, Login_C_Id, Is_Active, Is_Deleted, Emp_IsBlackList, Emp_WeekOff_Type, Company_Calendar_Id, Emp_Desig_Id, " . $main_result[2] . ", " . $main_result[3] . ", Cmp_Id, Emp_ReportingManager_Id, Is_Separate, HoldReason, Emp_Join_Date, Emp_BlacklistDate) VALUES ('" . $emp_code . "', '" . $emp_code . "', '" . $co_code . "', '.', '.', '.', '1', '0', '0', '0', '1', '1', '0', '0', '1', '" . $co_code . "', '5', ";
                    } else {
                    ...
                    if (mssql_query($payroll_insert_query, $oconn)) {
                        $emp_code++;
                        if ($main_result[13] == "WebPayMaster") {
                            $payroll_insert_query = "SET IDENTITY_INSERT tblEmployee OFF";
                            mssql_query($payroll_insert_query, $oconn);
                        }
                    } else {
            ...
            while ($cur = mysqli_fetch_row($result)) {
                if ($main_result[13] == "WebPayMaster") {
                    $payroll_query = "SET NOCOUNT ON UPDATE " . $main_result[0] . " SET " . $main_result[3] . " = '" . replaceString($cur[1], false) . "', Is_Separate = 0, HoldReason = '" . $cur[11] . "' ";
                } else {
                ...
                if ($oconn != "" && getRegister($txtMACAddress, 7) == "82") {
                    $payroll_query = $payroll_query . ", " . $main_result[9] . " = '" . $cur[9] . "'";
                    $payroll_query = $payroll_query . ", BranchCode = (SELECT BranchCode FROM tblBranch WHERE BranchName = '" . $cur[13] . "' AND BranchCode IS NOT NULL)";
                }
...
{
    if ($table != "tblSex") {
        if ($table != "tblEmployee" && $table != "tblPayDedGroup" && $table != "tblCategory" && getRegister($txtMACAddress, 7) != "103" && getRegister($txtMACAddress, 7) != "126") {
            ...
            if ($cur[0] != "") {
                ...
                if ($table == "tblBranch" && getRegister($txtMACAddress, 7) == "39") {
                    if ($cur[0] == "LORNA NIGERIA LIMITED") {
                        $query = "INSERT INTO " . $table . " (" . $f1 . ", BranchId, " . $f2 . ", CoCode) VALUES (" . $sub_cur[0] . ", 'LNL', 'LORNA NIGERIA LIMITED', '" . $co_code . "')";
                    } else {
                        if ($cur[0] == "GODREJ NIGERIA LIMITED") {
                            $query = "INSERT INTO " . $table . " (" . $f1 . ", BranchId, " . $f2 . ", CoCode) VALUES (" . $sub_cur[0] . ", 'GNL', 'GODREJ NIGERIA LIMITED', '" . $co_code . "')";
                        }
                    }
                } else {
                    if ($table == "tblDepartment") {
                        ...
                        if ($insert_cur[0] != replaceString($cur[0], false)) {
                            $query = "INSERT INTO " . $table . " (" . $f1 . ", DepartmentId, " . $f2 . ", CoCode, UserId, ProcessDay) VALUES (" . $sub_cur[0] . ", " . $sub_cur[0] . ", '" . replaceString($cur[0], false) . "', '" . $co_code . "', 1, 0)";
                        }
                    } else {
                        $query = "SELECT " . $f2 . " FROM " . $table . " WHERE " . $f2 . " = '" . replaceString($cur[0], false) . "'";
                        $insert_result = mssql_query($query, $aconn);
