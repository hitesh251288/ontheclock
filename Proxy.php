            ...
            for ($i = 0; $i < $count; $i++) { 
                if ($_POST["chk" . $i] != "") {   
					...
					for ($j = 0; $j < $_POST["txhDayCount"]; $j++) { 
						...
						if (isset($_POST["txhDay" . $j]) && isset($_POST["txhID" . $i]) && !empty($_POST["txhDay" . $j]) && !empty($_POST["txhID" . $i])) {
							$dayMasterID = (int)$_POST["txhDay" . $j]; // Cast to integer for safety
							$eID = (int)$_POST["txhID" . $i];         // Cast to integer for safety
							
							// Construct and execute the query
							$query = "DELETE FROM DayMaster WHERE DayMasterID = $dayMasterID AND e_id = $eID";
							updateIData($kconn, $query, true);
						} else {
							// Log or handle the error case when data is missing
							error_log("Missing DayMasterID or e_id for index $j");
						}
					}
					...
					for ($j = 0; $j < $_POST["txhWeekCount"]; $j++) { 
						// Validate and sanitize inputs
						if (isset($_POST["txhWeek" . $j]) && isset($_POST["txhID" . $i]) && !empty($_POST["txhWeek" . $j]) && !empty($_POST["txhID" . $i])) {
							$weekMasterID = (int)$_POST["txhWeek" . $j]; // Cast to integer for safety
							$eID = (int)$_POST["txhID" . $i];           // Cast to integer for safety

							// Construct and execute the query
							$query = "DELETE FROM WeekMaster WHERE WeekMasterID = $weekMasterID AND e_id = $eID";
							updateIData($lconn, $query, true);
						} else {
							// Log or handle the error case when data is missing
							error_log("Missing WeekMasterID or e_id for index $j");
						}
					}
					...
					for ($j = 0; $j < $_POST["txhAttendanceCount"]; $j++) { 
						// Validate and sanitize inputs
						if (isset($_POST["txhAttendance" . $j]) && isset($_POST["txhID" . $i]) && 
							!empty($_POST["txhAttendance" . $j]) && !empty($_POST["txhID" . $i])) {

							$attendanceID = (int)$_POST["txhAttendance" . $j]; // Cast to integer for safety
							$employeeID = (int)$_POST["txhID" . $i];           // Cast to integer for safety

							// Construct and execute the query
							$query = "DELETE FROM AttendanceMaster WHERE AttendanceID = $attendanceID AND EmployeeID = $employeeID";
							updateIData($mconn, $query, true);
						} else {
							// Log or handle the error case when data is missing
							error_log("Missing AttendanceID or EmployeeID for index $j");
						}
					} 
                }
            }
...
print "<form name='frm1' id='frm1' method='post' onSubmit='return checkProxySearch()' action='Proxy.php'><input type='hidden' name='act' value='searchRecord'>";
...
//    echo "<pre>";print_R($result);
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        for ($delCount = 0; $cur = mysqli_fetch_row($result); $delCount++) {
			print "<input type='hidden' name='txhDay" . $delCount . "' value='" . $cur[9] . "'>";
            if ($cur[3] == "") {
                $cur[3] = "&nbsp;";
            }
            if ($cur[6] == "") {
...
print "<input type='hidden' name='txtCounter' value='" . $count . "'>";
print "</div></div></div></div></div></form>";
include 'footer.php';
...
//	}else{
//		if (confirm("Mark Proxy Attendance for the Selected Employees for the Selected Date")){
//			if (confirm('NO Exit Terminal Entries would be Posted as Exit Terminal is NOT selected')){ alert(1);
//				//if (x.lstProxyShift.value != '' && x.lstExitTerminal.value == ''){
//					x.act.value='saveRecord';
//					x.btSubmit.disabled = true;
//					x.submit();
...
//    console.log("Form found?", x); // Debug: Check if form exists
    if (!x) {
        alert("Form not found! Check form name.");
        return false;
    }

//    console.log("txtTime:", x.txtTime?.value); // Debug: Check if element exists
//    console.log("lstDeptTerminal:", x.lstDeptTerminal?.value);

    if (!x.txtTime || !x.lstDeptTerminal || !x.lstExitTerminal || !x.lstProxyShift || !x.act || !x.btSubmit) {
        alert("Some form elements are missing. Check form structure.");
        return false;
    }
    ...
    if (x.txtTime.value == '' && x.lstDeptTerminal.value == '') {
        ...
        alert('Please select the Assigned Shift on Proxy Date OR enter a valid Proxy Time');
        x.lstProxyShift.focus();
        return false;
    } else if (x.lstProxyShift.value != '' && x.txtTime.value != '') {
        alert('Please select the Assigned Shift on Proxy Date OR enter a valid Proxy Time. Both Fields CANNOT be entered at the same Time.');
