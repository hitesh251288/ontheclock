...
$lstEmployeeID = $_POST["lstEmployeeID"];
if ($lstEmployeeID == "") {
...
print "<form name='frm1' method='post' onSubmit='return checkSearch()' action='AlterTime.php'><input type='hidden' name='act' value='searchRecord'>";
if ($excel != "yes") {
    print "<tr>";
...
if ($act == "searchRecord") {
    ...
    for ($date = ""; $cur = mysqli_fetch_row($result); $count++) {
        ...
        if ($prints != "yes") {
            displayDate($cur[5]);
            print "<td bgcolor='" . $bgcolor . "'><a title='Date'><input type='hidden' name='txhDate" . $count . "' value='" . $cur[5] . "'> <input name='txtDate" . $count . "' value='" . displayDate($cur[5]) . "' size='11' onBlur='javascript:check_valid_date(document.frm1.txtDate" . $count . ")' class='form-control-inner'></a></td>";
        } else {
            displayDate($cur[5]);
            print "<td bgcolor='" . $bgcolor . "'><a title='Date'><font face='Verdana' size='1'>" . displayDate($cur[5]) . "</font></a></td>";
        }
        if ($prints != "yes") {
            print "<td bgcolor='" . $bgcolor . "'><a title='Time'><input type='hidden' name='txhTime" . $count . "' value='" . $cur[6] . "'> <input name='txtTime" . $count . "' value='" . $cur[6] . "' size='7' onBlur='javascript:check_valid_time(document.frm1.txtTime" . $count . ")' class='form-control-inner'></a></td>";
        } else {
        ...
        if ($prints != "yes") {
            if (strpos($userlevel, $current_module . "E") !== false && $_SESSION[$session_variable . "LockDate"] < insertDate($txtFrom) && $_SESSION[$session_variable . "LockDate"] < insertDate($txtTo)) {
                print "<td bgcolor='" . $bgcolor . "'><input id='btEdit" . $cur[8] . "' type='button' value='Edit' onClick='javascript:openWindow(1, " . $cur[8] . ")' class='btn btn-primary'></td>";
            } else {
    ...
    if ($excel != "yes") {
        ...
        if ($prints != "yes") {
            if (strpos($userlevel, $current_module . "E") !== false && $_SESSION[$session_variable . "LockDate"] < insertDate($txtFrom) && $_SESSION[$session_variable . "LockDate"] < insertDate($txtTo)) {
                print "<tr>";
                $query = "SELECT id, name from tgroup ORDER BY name";
                displayList("lstEditShift", "Change the Shift of the selected Record(s) to: <br><font size='1'>(<b>Leave selection as Blank if Shift Change is NOT required</b>)</font>", $lstEditShift, $prints, $conn, $query, "", "50%", "50%");
                print "</tr>";
                print "<tr>";
                $query = "SELECT id, name from tgate ORDER BY name";
                displayList("lstEditTerminal", "Change the Terminal of the selected Record(s) to: <br><font size='1'>(<b>Leave selection as Blank if Terminal Change is NOT required</b>)</font>", $lstEditTerminal, $prints, $conn, $query, "", "50%", "50%");
                print "</tr>";
                print "<tr><td>&nbsp;</td><td><input type='button' value='Save Changes' onClick='javascript:saveChanges()' class='btn btn-primary'></td></tr>";
            }
            if (strpos($userlevel, $current_module . "D") !== false && $_SESSION[$session_variable . "LockDate"] < insertDate($txtFrom) && $_SESSION[$session_variable . "LockDate"] < insertDate($txtTo)) {
                print "<tr><td>&nbsp;</td><td><input type='button' value='Delete Selected' onClick='javascript:deleteSelected()' class='btn btn-primary'></td></tr>";
            }
            if (strpos($userlevel, $current_module . "A") !== false && $_SESSION[$session_variable . "LockDate"] < insertDate($txtFrom) && $_SESSION[$session_variable . "LockDate"] < insertDate($txtTo) && insertDate($txtFrom) < insertToday() && insertDate($txtTo) < insertToday()) {
                print "<tr><td>&nbsp;</td><td><input id='btFSC' type='button' value='Fix SINGLE Clockings' onClick='javascript:fixSingleClocking()' class='btn btn-primary'></td></tr>";
            }
            print "<tr><td>&nbsp;</td><td><input type='button' value='Print Report' onClick='checkPrint(0)' class='btn btn-primary'>&nbsp;&nbsp;<input type='button' value='Excel' onClick='checkPrint(1)' class='btn btn-primary'></td></tr>";
        }
        print "</table></center>";
    }
}
