...
$queryData =  date('Ym');
echo $sql = "SELECT CONVERT(VARCHAR(19), event_time, 120),user_id,terminal_id from dbo.auth_logs_$queryData where user_id != '-1'";
...
//            echo "<br>";
//            $sqlDatas[] = $dboData[$i][2];
//            echo "<pre>";
//            print_R($sqlDatas);
//        }
////        echo "<pre>";print_R($dboData[$i]);
//    }
//}
//echo "Count" . $j;
//for($j=0;$j<=count($sqlData);$j++){
//    
////    echo "<pre>";print_R($dboData[$j]);
//    echo $dboData[$j][0] ."==". $sqlData[$j]['userid'];
//    echo "<br>";
//    if($dboData[$k][0] === $sqlData[$j]['userid']){
////        $sqlData[$j]['e_group'] = $dboData[$j][2];
//        echo "<pre>";print_R($sqlData[$j]);
//    }
//}
//echo "<pre>";print_R($sqlData);
...
//            $j++;
//            echo $sqlDatas['userid'] . "=" . $dboData[$i][0];
//            echo $sqlDatas['userid'] ."!=". $mysqlData[$i]['e_id'];
//            echo "<br>";
            if (!in_array($mysqlData[$i]['e_id'], $sqlDatas['userid']) && !in_array($mysqlData[$i]['mdate'], $sqlDatas['sdate']) && !in_array($sqlDatas['mtime'], $sqlDatas['stime']) && !in_array($mysqlData[$i]['g_id'], $sqlDatas['t_id'])) {
                echo $addQuery = "INSERT INTO tenter (e_date,e_time,g_id,e_id,e_group) VALUES ('" . $sqlDatas['sdate'] . "','" . $sqlDatas['stime'] . "','" . $sqlDatas['t_id'] . "','" . $sqlDatas['userid'] . "','" . $dboData[$i][2] . "')";
                echo "<br>";
                $addResult = updateIData($iconn, $addQuery, true);
            }
