    ...
    if ($aconn != "") {
        ...
        while (odbc_fetch_into($result, $cur)) {
            ...
            if ($sub_result[0] == $cur[0]) {
                $query = "UPDATE tuser SET name = '" . replaceString($cur[1], false) . "', idno = '" . $cur[3] . "', phone = '" . replaceString($cur[7], false) . "' WHERE id = '" . $cur[0] . "'";
                if (!updateIData($iconn, $query, true)) {
                    echo "\n\rError: " . $query;
                }
            } else {
                $query = "INSERT INTO tuser (id, name, idno, reg_date, phone) VALUES ('" . $cur[0] . "', '" . replaceString($cur[1], false) . "', '" . replaceString($cur[3], false) . "'";
                $query .= ", '" . insertParadoxDate(substr($cur[7], 0, 10)) . "0100', '" . replaceString($cur[7], false) . "')";
                if (!updateIData($iconn, $query, true)) {
                    echo "\n\rError: " . $query;
                    exit;
                }
