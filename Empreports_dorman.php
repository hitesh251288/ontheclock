...
if (!checkSession($userlevel, $current_module)) {
    header("Location: " . $config["REDIRECT"] . "?url=Empreports_dorman.php&message=Session Expired or Security Policy Violated");
}
$conn = openConnection();
$unisConn = mysqli_connect('localhost', 'root', 'namaste', 'dorman_unis', '3306');
...
$unisTuser = "SELECT L_ID, C_Unique FROM tuser";
$unisResult = mysqli_query($unisConn, $unisTuser);
$unisData = []; 
while ($unisRow = mysqli_fetch_assoc($unisResult)) {
    $unisData[$unisRow['L_ID']] = $unisRow;
}
if ($act == "searchRecord") {
