...
<div class="page-breadcrumb">
                    <div class="row">
                        <div class="col-12 d-flex no-block align-items-center">
                            <h4 class="page-title">Late In/ Early Out Report</h4>
...
//            print "<tr><td>&nbsp;</td><td><font face='Verdana' size='1'><b>Selected Options</b></font></td></tr>";
        ...
        <div class="row">
            ...
            <div class="col-2">
                <?php 
                displayTextbox("ex3", "Minute(s) <b>MORE</b> THAN: ", $ex3, $prints, 5, "", "");
                ?>
            </div>
            <div class="col-2">
                <?php 
...
//                }
            ...
            if ($_SESSION[$session_variable . "ExitTerminal"] == "Yes") {
                $query = "SELECT tuser.id, tuser.name, tuser.dept, tuser.company, tgroup.name, tenter.e_date, tenter.e_time, tgate.name, tuser.idno, tuser.remark, tgroup.Close, tgroup.NightFlag FROM tuser, tgroup, tenter, tgate WHERE tenter.e_group = tgroup.id AND tenter.e_id = tuser.id AND tenter.g_id = tgate.id AND tgroup.Close >= '0000' AND tuser.id > 0 AND tgate.exit = 0 " . $_SESSION[$session_variable . "DeptAccessQuery"] . " " . $_SESSION[$session_variable . "DivAccessQuery"] . "  AND tuser.id IN (SELECT DISTINCT(tuser.id) FROM tuser, tenter, tgate WHERE tenter.e_id = tuser.id AND tenter.g_id = tgate.id AND tgate.exit = 1) AND ((tenter.e_time < '240000' AND tgroup.NightFlag =0) OR (tenter.e_time < '" . $NightShiftMaxOutTime . "00' AND tgroup.NightFlag = 1)) ";
            } else {
                $query = "SELECT tuser.id, tuser.name, tuser.dept, tuser.company, tgroup.name, tenter.e_date, tenter.e_time, tgate.name, tuser.idno, tuser.remark, tgroup.Close, tgroup.NightFlag FROM tuser, tgroup, tenter, tgate WHERE tenter.e_group = tgroup.id AND tenter.e_id = tuser.id AND tgroup.Close >= '0000' AND tuser.id > 0 AND tenter.g_id = tgate.id " . $_SESSION[$session_variable . "DeptAccessQuery"] . " " . $_SESSION[$session_variable . "DivAccessQuery"] . " AND ((tenter.e_time < '240000' AND tgroup.NightFlag =0) OR (tenter.e_time < '" . $NightShiftMaxOutTime . "00' AND tgroup.NightFlag = 1)) ";
            }
            if ($lstShift != "") {
                $query = $query . " AND tgroup.id = " . $lstShift;
            }
...
//                }
                while ($cur = mysqli_fetch_row($result)) {
                    if ($cur[0] == $last_id && $cur[5] == $last_date) {
                        ...
                        if ($data6 < getEarlyTime($date_count, $data10, $ex3) && strlen($data5) == 8 && 0 < $ecount) {
                            ...
                            print "<tr><td><a title='ID'><input type='hidden' name='txh" . $count . "' value='" . $data0 . "'><font face='Verdana' size='1'>" . addZero($data0, $_SESSION[$session_variable . "EmployeeCodeLength"]) . "</font></a></td> <td><a title='Name'><font face='Verdana' size='1'>" . $data1 . "</font></a></td> <td><a title='" . $_SESSION[$session_variable . "IDColumnName"] . "'><font face='Verdana' size='1'>" . $data8 . "</font></a></td> <td><a title='Dept'><font face='Verdana' size='1'>" . $data2 . "</font></a></td> <td><a title='Div/Desg'><font face='Verdana' size='1'>" . $data3 . "</font></a></td> <td><a title='Rmk'><font face='Verdana' size='1'>" . $data9 . "</font></a></td> <td><a title='Shift'><font face='Verdana' size='1'>" . $data4 . "</font></a></td> <td><a title='Close'><font face='Verdana' size='1'>" . displayVirdiTime($data10 . "00") . "</font></a></td> <td><font face='Verdana' size='1'>&nbsp;</font></td> <td><a title='Date'><font face='Verdana' size='1'>" . displayDate($data5) . "</font></a></td> <td><a title='Depart'><font face='Verdana' size='1'><b>" . displayVirdiTime($data6) . "</b></font></a></td> <td><a title='Early Minutes'><font face='Verdana' size='1'><b>" . getEarlyMin($date_count, $data10, $data6) . "</b></font></a></td> <td><a title='Terminal'><font face='Verdana' size='1'>" . $data7 . "</font></a></td></tr>";
                            $count++;
                        }
                        $last_id = $cur[0];
                        $last_date = $cur[5];
    ...
    if ($prints != "yes") {
        print "<br><input type='button' value='Print Report' onClick='checkPrint(0)' class='btn btn-primary'>&nbsp;&nbsp;<input type='button' class='btn btn-primary' value='Excel' onClick='checkPrint(1)'>";
    }
    print "</p>";
}
print "</form>";
