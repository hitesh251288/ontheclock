...
class POP3
{
    ...
    protected function checkResponse($string)
    {
        if (substr($string, 0, 3) !== "+OK") {
            $this->setError(array("error" => "" . "Server reported an error: " . $string, "errno" => 0, "errstr" => ""));
            return false;
        }
        return true;
    }
    ...
    {
        $this->setError(array("error" => "Connecting to the POP3 server raised a PHP warning: ", "errno" => $errno, "errstr" => $errstr, "errfile" => $errfile, "errline" => $errline));
    }
}
