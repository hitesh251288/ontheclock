...
if (!checkSession($userlevel, $current_module)) {
    header("Location: " . $config["REDIRECT"] . "?url=MonthlyAttendanceSummary.php&message=Session Expired or Security Policy Violated");
}
$conn = openConnection();
$act = $_GET["act"];
if ($act == "") {
...
//print "<html><title>Monthly Summary Report</title>";
if ($prints != "yes") {
//    print "<body>";
        ...
        if ($lstGroupBy != "") { 
            ...
            for ($date_count = insertDate($txtFrom); $date_count <= insertDate($txtTo); $date_count++) {
                if (checkdate(substr($date_count, 4, 2), substr($date_count, 6, 2), substr($date_count, 0, 4))) {
                    $query = "SELECT tuser.id, tuser.name, tuser.dept, tuser.company, tgroup.name, tenter.e_date, MIN(tenter.e_time) as InTime,MAX(tenter.e_time) as OutTime, tgate.name, tuser.idno, tuser.remark, tuser.phone, tgroup.Start, tgroup.Close,COUNT(DISTINCT tenter.e_date) AS DaysWorked FROM tuser, tgroup, tenter, tgate WHERE tenter.e_group = tgroup.id AND tenter.e_id = tuser.id AND tenter.g_id = tgate.id AND tgate.exit = 0 AND ((tenter.e_time > '" . $cutoff . "00' AND tgroup.NightFlag = 1) OR (tenter.e_time > '000000' AND tgroup.NightFlag = 0)) " . $_SESSION[$session_variable . "DeptAccessQuery"] . " " . $_SESSION[$session_variable . "DivAccessQuery"] . " ";
                    if ($lstShift != "") {
...
//                       $query = $query . " AND tenter.e_date = '" . $date_count . "' GROUP BY tuser.id,tenter.e_date";
                    ...
                    if (mysqli_num_rows($result) > 0) {                       
                        print "<thead><tr><td><font face='Verdana' size='1'>&nbsp;</font></td> <td><font face='Verdana' size='1'>&nbsp;</font></td> <td><font face='Verdana' size='1'>&nbsp;</font></td><td><font face='Verdana' size='1'>&nbsp;</font></td><td><font face='Verdana' size='1'>&nbsp;</font></td><td><font face='Verdana' size='1'>&nbsp;</font></td></tr>";
                        print "<tr><td><font face='Verdana' size='2'>ID</font></td> <td><font face='Verdana' size='2'>Surname</font></td><td><font face='Verdana' size='2'>OtherName</font></td><td><font face='Verdana' size='2'>Dept</font></td><td><font face='Verdana' size='2'>Days Worked</font></td><td><font face='Verdana' size='2'>Salary</font></td></thead>";
...
//                                            echo "<br>";
                                            ...
                                            $bfTime = $cur[12] - 30;
                                            $fullName = $cur[1];
                                                $firstSpacePos = strpos($fullName, ' ');
                                                if ($firstSpacePos !== false) {
                                                    // Extract the surname (before the first space)
                                                    $surname = substr($fullName, 0, $firstSpacePos);

                                                    // Extract the rest of the name (after the first space)
                                                    $otherNames = substr($fullName, $firstSpacePos + 1);

                                                    // Combine them with the surname first
                                                    $rearrangedName = $surname . ' ' . $otherNames;
                                                } else {
                                                    // No space found, so consider the whole name as the surname or handle as needed
                                                    $rearrangedName = $fullName;
                                                }
                                            if(date("H:i:s", strtotime($cur[6])) >= $startTime){     
//                                                echo "Hey";
                                                print "<tr><td><a title='ID'><input type='hidden' name='txh" . $count . "' value='" . $cur[0] . "'><font face='Verdana' size='1' color='red'>" . addZero($cur[0], $_SESSION[$session_variable . "EmployeeCodeLength"]) . "</font></a></td> <td><a title='Name'><font face='Verdana' size='1'>" . $surname . "</font></a></td><td><a title='otherName'><font face='Verdana' size='1'>" . $otherNames . "</font></a></td><td><a title='Dept'><font face='Verdana' size='1'>" . $cur[2] . "</font></a></td><td><a title='OutTime'><font face='Verdana' size='1'>" . $cur[14] . "</font></a></td><td><a title='Salary'><font face='Verdana' size='1'></font></a></td>";
